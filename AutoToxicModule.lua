...
function AutoToxicModule:Enable()
    ...
    -- Table pour stocker les connexions (pour les nettoyer plus tard)
    local connections = {}
    -- Table pour suivre les cibles surveillées (clé: joueur, valeur: true si surveillé)
    local monitoredTargets = {}
    -- Table pour suivre si un message a déjà été envoyé pour une instance de HP bas
    ...
    local function monitorTargetHealth(targetPlayer)
        ...
        healthMonitorConnection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            if not getgenv().Rake.Settings.Misc.AdvancedMisc.AutoToxic then
        ...
        characterAddedConnection = targetPlayer.CharacterAdded:Connect(function(newCharacter)
            ...
            if getgenv().Rake.Settings.Misc.AdvancedMisc.AutoToxic and targetPlayer == Plr and enabled and getgenv().Rake.Settings.Misc.ForceHit then
    ...
    -- Fonction pour désactiver (sera appelée lors de la désactivation)
    AutoToxicModule.Disable = function()
