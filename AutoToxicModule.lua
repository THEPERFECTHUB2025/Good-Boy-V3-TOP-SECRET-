...
local function monitorTargetHealth(targetPlayer, getPlr, getEnabled, getForceHit, getSilentAimTarget)
    ...
    characterAddedConnection = targetPlayer.CharacterAdded:Connect(function(newCharacter)
        ...
        if isEnabled and (targetPlayer == Plr or targetPlayer == SilentAimTarget) then
            ...
            monitorTargetHealth(targetPlayer, getPlr, getEnabled, getForceHit, getSilentAimTarget)
        end
    end)
...
function AutoToxicModule:Enable(getPlr, getEnabled, getForceHit, getSilentAimTarget)
    if isEnabled then
    ...
    if not getPlr or not getEnabled or not getForceHit or not getSilentAimTarget then
        error("AutoToxicModule:Enable requires getPlr, getEnabled, getForceHit, and getSilentAimTarget functions as arguments")
    end
    ...
    targetMonitorConnection = game:GetService("RunService").Heartbeat:Connect(function()
        ...
        if currentTarget and not monitoredTargets[currentTarget] then
            ...
            monitorTargetHealth(currentTarget, getPlr, getEnabled, getForceHit, getSilentAimTarget)
        else
