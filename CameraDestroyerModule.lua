...
function CameraDestroyerModule:UpdateBillboardState()
    if getgenv().Rake.Settings.Misc.CameraDestroyer then
        ...
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head") then
            ...
            print("BillboardGui updated: Adornee set to", LocalPlayer.Character.Head, "Enabled:", billboardGui.Enabled, "Text:", stateLabel.Text)
        else
            print("Failed to update BillboardGui: Character or Head not found")
        end
    else
        -- Toggle is disabled, hide the BillboardGui
        billboardGui.Enabled = false
        billboardGui.Adornee = nil
        print("BillboardGui disabled: Toggle is off")
    end
end
...
function CameraDestroyerModule:Cleanup()
    ...
    print("CameraDestroyerModule cleaned up")
end
...
LocalPlayer.CharacterAdded:Connect(function(character)
    -- Wait for the character to be fully loaded (Head part available)
    local head = character:WaitForChild("Head", 5) -- Wait up to 5 seconds for the Head
    if head then
        if getgenv().DC then
        ...
        print("Character respawned: BillboardGui updated")
    else
        warn("Failed to find Head after respawn within 5 seconds")
    end
end)
...
if LocalPlayer.Character then
    ...
    if head then
        CameraDestroyerModule:UpdateBillboardState()
        print("Initial character found: BillboardGui updated")
    else
        print("Initial character found, but Head not available yet")
    end
end
