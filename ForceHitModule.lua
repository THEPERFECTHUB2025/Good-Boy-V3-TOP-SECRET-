...
local ForceHitModule = {
    ...
    LastShotTime = 0, -- Add this for firing delay
    FiringDelay = 0.1, -- Add this for firing delay (adjust as needed)
}
...
local function UpdateTargetAndHighlight()
    if not ForceHitModule.ManuallyEnabled then
        print("[UpdateTargetAndHighlight] ForceHit not manually enabled, disabling")
    ...
    -- Select a new target only if none exists
    if not ForceHitModule.TargetPlayer then
        print("[UpdateTargetAndHighlight] No target set, selecting new target")
        ...
        -- If a target is selected, set up respawn handling
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if IsPlayerWhitelisted(ForceHitModule.TargetPlayer) then
            print("[UpdateTargetAndHighlight] Target is whitelisted, selecting new target:", ForceHitModule.TargetPlayer.Name)
            ForceHitModule.TargetPlayer = nil
            ForceHitModule.SelectedTarget = nil
            local ClosestPart, ClosestCharacter, ClosestPlayer = GetClosestPlayer()
            ForceHitModule.SelectedTarget = ClosestPart
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                -- Update the selected target part if the character is valid
                ForceHitModule.SelectedTarget = hitPart
                print("[UpdateTargetAndHighlight] Target updated:", ForceHitModule.TargetPlayer.Name, "HitPart:", ForceHitModule.HitPart)

                local health = humanoid.Health
                if health >= 0.5 and health <= 20 then
                    if ForceHitModule.Enabled then
                        print("[UpdateTargetAndHighlight] Target health between 0.5 and 20, disabling ForceHit")
                        ForceHitModule.Enabled = false
                    end
                elseif health > 20 then
                    if not ForceHitModule.Enabled then
                        print("[UpdateTargetAndHighlight] Target health > 20, enabling ForceHit")
                ...
                if localPosition and hitPart then
                    ...
                    -- Toggle BlankShots based on distance (0 to 200 studs)
                    if distance <= 200 then
                        ForceHitModule.BlankShots = true
                        print("[UpdateTargetAndHighlight] Distance <= 200 studs, BlankShots enabled")
                    else
                        ForceHitModule.BlankShots = false
                        print("[UpdateTargetAndHighlight] Distance > 200 studs, BlankShots disabled")
                    end
                    ...
                    if ForceHitModule.WallCheckEnabled and IsTargetBehindWall(localPosition, targetPosition) then
                        if ForceHitModule.Enabled then
                            print("[UpdateTargetAndHighlight] Target behind wall, disabling ForceHit")
                            ForceHitModule.Enabled = false
                        end
                    else
                        if not ForceHitModule.Enabled and health > 20 then
                            print("[UpdateTargetAndHighlight] Target not behind wall and health > 20, enabling ForceHit")
                            ForceHitModule.Enabled = true
                        end
                    end
                end
            else
                -- Target is invalid (dead, no HitPart, or has ForceField), but don't clear the target
                print("[UpdateTargetAndHighlight] Target invalid (no HitPart, dead, or has ForceField), waiting for respawn:", ForceHitModule.TargetPlayer.Name)
                ForceHitModule.SelectedTarget = nil -- Clear the selected part, but keep the TargetPlayer
            end
        else
            -- Target character not found, but don't clear the target
            print("[UpdateTargetAndHighlight] Target character not found, waiting for respawn:", ForceHitModule.TargetPlayer.Name)
            ForceHitModule.SelectedTarget = nil -- Clear the selected part, but keep the TargetPlayer
        end
    end
    ...
    if not ForceHitModule.Enabled then
        if ForceHitModule.Highlight then
            ForceHitModule.Highlight.Enabled = false
            print("[UpdateTargetAndHighlight] ForceHit disabled, highlight turned off")
        end
    end
    ...
    if character and ForceHitModule.Highlight then
        ...
        print("[UpdateTargetAndHighlight] Highlight enabled for target:", ForceHitModule.TargetPlayer and ForceHitModule.TargetPlayer.Name or "None")
    else
        if ForceHitModule.Highlight then
            ForceHitModule.Highlight.Enabled = false
            print("[UpdateTargetAndHighlight] Highlight disabled, no valid target")
        end
    end
...
-- Update the Cleanup function to clear the whitelist
    function ForceHitModule:Cleanup()
        ...
        self.OriginalEnabledState = nil -- Clear the stored state on cleanup
...
function ForceHitModule:UpdateTargetInfoUI()
    if not self.TargetInfoUI then
        print("[UpdateTargetInfoUI] Creating TargetInfoUI")
        CreateTargetInfoUI()
    end
    ...
    if not getgenv().Rake.Settings.Misc.TargetInfoEnabled or not self.ManuallyEnabled or not self.TargetPlayer then
        if ui.Frame.Visible then
            print("[UpdateTargetInfoUI] Hiding UI: TargetInfoEnabled=", getgenv().Rake.Settings.Misc.TargetInfoEnabled, "ManuallyEnabled=", self.ManuallyEnabled, "TargetPlayer=", self.TargetPlayer)
    ...
    if not humanoid or not targetRootPart or not localRootPart then
        if ui.Frame.Visible then
            print("[UpdateTargetInfoUI] Hiding UI: Missing humanoid, targetRootPart, or localRootPart")
            ui.Frame.Visible = true
            local particleFrame = Instance.new("Frame")
            particleFrame.Size = UDim2.new(1, 0, 1, 0)
            particleFrame.BackgroundTransparency = 1
    ...
    print("[UpdateTargetInfoUI] Updating UI for target:", targetPlayer.Name)
    ...
    if not ui.Frame.Visible then
        print("[UpdateTargetInfoUI] Showing UI for target:", targetPlayer.Name)
        ui.Frame.Visible = true
                ...
                if Part and not ForceField then
                    ...
                    if OnScreen then
                        ...
                        if Score < ClosestToMouse then
                            ...
                            ClosestPart = Part
                            ClosestCharacter = Character
                            print("[GetClosestPlayer] Selected target:", Player.Name, "Distance to mouse:", MouseDistance)
                        end
                    else
                        print("[GetClosestPlayer] Player not on screen:", Player.Name)
                    end
                else
                    print("[GetClosestPlayer] Player skipped - No HitPart or has ForceField:", Player.Name)
                end
            else
                print("[GetClosestPlayer] Player skipped - No character or dead:", Player.Name)
            end
        else
            print("[GetClosestPlayer] Player skipped - Whitelisted or self:", Player.Name)
        end
    end

    if not ClosestPlayer then
        print("[GetClosestPlayer] No valid target found")
    end
    return ClosestPart, ClosestCharacter, ClosestPlayer
end
...
local function UpdateTracer()
    if not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then
        if ForceHitModule.Tracer then
            ...
            print("[UpdateTracer] Tracer destroyed: ForceHit not enabled or no target")
        end
    ...
    if not localRootPart then
        if ForceHitModule.Tracer then
            ForceHitModule.Tracer:Destroy()
            ForceHitModule.Tracer = nil
            print("[UpdateTracer] Tracer destroyed: Local HumanoidRootPart not found")
        end
        if ForceHitModule.Attachment0 then
            ForceHitModule.Attachment0:Destroy()
            ForceHitModule.Attachment0 = nil
    ...
    if not targetHitPart then
        if ForceHitModule.Tracer then
            ForceHitModule.Tracer:Destroy()
            ForceHitModule.Tracer = nil
            print("[UpdateTracer] Tracer destroyed: Target HitPart not found")
        end
        if ForceHitModule.Attachment0 then
            ForceHitModule.Attachment0:Destroy()
            ForceHitModule.Attachment0 = nil
    ...
    if not ForceHitModule.Tracer then
        ...
        print("[UpdateTracer] Tracer created: From", localRootPart, "to", targetHitPart)
    end
    ...
    if ForceHitModule.Attachment0 and ForceHitModule.Attachment0.Parent ~= localRootPart then
        ...
        print("[UpdateTracer] Attachment0 updated to new localRootPart")
    end
    ...
    if ForceHitModule.Attachment1 and ForceHitModule.Attachment1.Parent ~= targetHitPart then
        ...
        print("[UpdateTracer] Attachment1 updated to new targetHitPart")
    end
end
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if ForceHitModule.AutoTargetAll then
                    ...
                    if currentTime - ForceHitModule.LastTargetSwitchTime >= 0.5 then
                        ...
                        if newPlayer and newPlayer ~= ForceHitModule.TargetPlayer then
                            print("[UpdateTargetAndHighlight] AutoTargetAll: Switching target to", newPlayer.Name)
                ...
                print("[UpdateTargetAndHighlight] Target invalid (no HitPart, dead, or has ForceField), clearing target:", ForceHitModule.TargetPlayer.Name)
                ForceHitModule.SelectedTarget = nil
                ForceHitModule.TargetPlayer = nil
            end
        else
            print("[UpdateTargetAndHighlight] Target character not found, clearing target")
            ForceHitModule.SelectedTarget = nil
            ForceHitModule.TargetPlayer = nil
        end
    end
...
local function CheckTargetAfterRespawn()
    ...
    -- Disconnect any existing respawn connection to avoid duplicates
    if ForceHitModule.Connections["TargetRespawn"] then
        ForceHitModule.Connections["TargetRespawn"]:Disconnect()
    end
    -- Set up a new connection to handle respawn
    ForceHitModule.Connections["TargetRespawn"] = player.CharacterAdded:Connect(function(newCharacter)
        if not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then return end
        ...
        if hitPart and humanoid and humanoid.Health > 0 then
            ForceHitModule.SelectedTarget = hitPart
            print("[CheckTargetAfterRespawn] Target respawned, reassigning target:", player.Name, "HitPart:", ForceHitModule.HitPart)
            -- Force an update to ensure visuals and logic kick in
...
function ForceHitModule:Enable()
    ...
    if not self.TargetPlayer then
        ...
        print("[ForceHitModule:Enable] Target set to:", ClosestPlayer and ClosestPlayer.Name or "None")
        CheckTargetAfterRespawn()
    end
