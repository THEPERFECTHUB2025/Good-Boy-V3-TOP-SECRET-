...
-- GetClosestPlayer (already excludes whitelisted players)
...
-- GetClosestPlayerByDistance (already excludes whitelisted players)
...
-- Modified UpdateTargetAndHighlight to keep target locked but control shooting based on distance
local function UpdateTargetAndHighlight()
    if not ForceHitModule.ManuallyEnabled then
        ...
        ForceHitModule.TargetPlayer = nil -- Clear target when manually disabled
        UpdateTracer()
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                elseif health > 20 then
                    -- We don't disable the target here; we just control shooting
                ...
                if localPosition and hitPart then
                    ...
                    -- Only enable shooting if the target is within 200 meters
                    if distance > 714.2 then -- 200 meters ≈ 714.2 studs
                        if ForceHitModule.Enabled then
                            ForceHitModule.Enabled = false
                        end
                    elseif distance <= 714.2 then
                    ...
                    if ForceHitModule.WallCheckEnabled and IsTargetBehindWall(localPosition, targetPosition) then
                        ...
                        if not ForceHitModule.Enabled and health > 20 and distance <= 714.2 then
                            ForceHitModule.Enabled = true
                        end
                    end

                    -- Removed the IsTargetInVoid check to keep the target locked even in the void
                    -- The distance check above will prevent shooting if they're too far
                end
            else
                -- If the target's character is invalid (e.g., they died), clear the target
                ForceHitModule.SelectedTarget = nil
                ForceHitModule.TargetPlayer = nil
            end
        else
            -- If the target player has no character, clear the target
            ForceHitModule.SelectedTarget = nil
            ForceHitModule.TargetPlayer = nil
        end
    end
...
-- Update Tracer to work at infinite distance
...
-- Modify TargetStrafe to remove distance and void checks
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    -- Define strafe parameters
    local radius = 0 -- Distance from the target (in studs)
    ...
    local offset = Vector3.new(
        math.cos(ForceHitModule.StrafeAngle) * radius,
        0, -- We'll adjust Y separately
        math.sin(ForceHitModule.StrafeAngle) * radius
    )

    -- Calculate the new position around the target
    local newPosition = targetRootPart.Position + offset

    -- Adjust the height to match the target's Y level smoothly
    local heightOffset = targetRootPart.Position.Y - localRootPart.Position.Y
    local adjustedPosition = newPosition + Vector3.new(0, heightOffset + 1, 0) -- Add 1 stud to avoid clipping into the ground

    -- Update the player's position and orientation
    localRootPart.CFrame = CFrame.new(adjustedPosition, targetRootPart.Position)
end)

-- AutoStomp (already excludes whitelisted players)
...
-- BlankShots (already excludes whitelisted players)
...
local function CheckTargetAfterRespawn()
    ...
    player.CharacterAdded:Connect(function(newCharacter)
        ...
        -- Re-lock onto the target after they respawn
