        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if localPosition and hitPart then
                    ...
                    -- Removed distance check to allow ForceHit to stay active at infinite distance
                    -- Removed void check to allow ForceHit to stay active even in the void

                    -- Only disable shooting if behind a wall (if WallCheckEnabled is true)
        ...
        if AimPart then
            ...
            if localRootPart then
                ...
                if distance > 714.2 then -- 200 meters ≈ 714.2 studs
                    return OriginalNameCall(Object, ...) -- Don't modify the shot if out of range
                end
            end
        ...
        if not ForceField then
            if AimPart and MainEvent then
                ...
                if localRootPart then
                    local distance = (localRootPart.Position - AimPart.Position).Magnitude
                    if distance > 714.2 then -- 200 meters ≈ 714.2 studs
                        return -- Don't shoot if the target is too far
                    end
                end
