...
local ForceHitModule = {
    ...
    Tracer = nil, -- Référence au Beam du Tracer
    TracerAttachment0 = nil, -- Attachment pour le joueur local
    TracerAttachment1 = nil -- Attachment pour la cible
}
...
-- Fonction pour créer un Tracer entre le joueur local et la cible
local function createTracer(plr)
    if plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        -- Supprimer l'ancien Tracer s'il existe
        if ForceHitModule.Tracer then
            ForceHitModule.Tracer:Destroy()
            ForceHitModule.Tracer = nil
        end
        if ForceHitModule.TracerAttachment0 then
            ForceHitModule.TracerAttachment0:Destroy()
            ForceHitModule.TracerAttachment0 = nil
        end
        if ForceHitModule.TracerAttachment1 then
            ForceHitModule.TracerAttachment1:Destroy()
            ForceHitModule.TracerAttachment1 = nil
        end

        -- Créer le Beam
        local tracer = Instance.new("Beam")
        tracer.Parent = game.CoreGui -- Placer dans CoreGui pour qu'il soit visible à travers les murs
        tracer.FaceCamera = true
        tracer.Color = ColorSequence.new(Color3.new(1, 1, 1)) -- Couleur blanche
        tracer.Width0 = 0.1
        tracer.Width1 = 0.1
        ...
        attachment1.Parent = plr.Character.HumanoidRootPart
        ...
        ForceHitModule.TracerAttachment0 = attachment0
        ForceHitModule.TracerAttachment1 = attachment1

        print("Tracer: Créé entre " .. LocalPlayer.Name .. " et " .. plr.Name)
    end
end
...
local function UpdateTargetAndHighlight()
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                -- Créer ou mettre à jour le Tracer si ForceHit est activé
                if ForceHitModule.Enabled then
                    createTracer(ForceHitModule.TargetPlayer)
                end
            else
                ForceHitModule.SelectedTarget = nil
                -- Désactiver le Tracer si la cible n'est plus valide
                if ForceHitModule.Tracer then
                    ForceHitModule.Tracer:Destroy()
                    ForceHitModule.Tracer = nil
                end
            ...
            ForceHitModule.SelectedTarget = nil
            -- Désactiver le Tracer si le personnage de la cible n'existe plus
            if ForceHitModule.Tracer then
                ForceHitModule.Tracer:Destroy()
                ForceHitModule.Tracer = nil
            end
    ...
    -- Si ForceHit est désactivé (par la logique des HP, de la distance, du wall check ou autre), désactiver le Highlight et le Tracer
    if not ForceHitModule.Enabled then
        Highlight.Enabled = false
        if ForceHitModule.Tracer then
            ForceHitModule.Tracer:Destroy()
