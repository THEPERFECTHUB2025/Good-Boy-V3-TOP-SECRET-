...
local ForceHitModule = {
    Enabled = false, -- État effectif pour la logique de tir (peut être modifié par la logique des HP, de la distance ou du wall check)
    ManuallyEnabled = false, -- État défini par l'utilisateur via le keybind ou l'UI (contrôle le Highlight et le Tracer)
    ...
    WallCheckEnabled = true, -- Option pour activer/désactiver le Wall Check (pour la logique de tir uniquement)
...
-- Fonction pour créer ou mettre à jour le Tracer
local function UpdateTracer()
    ...
    -- Vérifier que le joueur local et la cible ont un HumanoidRootPart
    local localCharacter = LocalPlayer.Character
    ...
    if not localRootPart or not targetRootPart then
        print("Tracer: Personnage ou cible non valide - LocalRootPart:", localRootPart, "TargetRootPart:", targetRootPart)
        -- Supprimer le Tracer s'il existe
        if ForceHitModule.Tracer then
            ForceHitModule.Tracer:Destroy()
            ForceHitModule.Tracer = nil
    ...
    if not ForceHitModule.Tracer then
        ...
        print("Tracer: Créé entre " .. LocalPlayer.Name .. " et " .. ForceHitModule.TargetPlayer.Name)
    else
        -- Si le Tracer existe, vérifier que les Attachments sont toujours valides
        ...
        if ForceHitModule.Attachment1 and ForceHitModule.Attachment1.Parent ~= targetRootPart then
            print("Tracer: Mise à jour de Attachment1")
            ForceHitModule.Attachment1:Destroy()
            local newAttachment1 = Instance.new("Attachment")
            newAttachment1.Parent = targetRootPart
...
local function UpdateTargetAndHighlight()
    -- Si l'utilisateur a désactivé manuellement, désactiver tout
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                -- Vérifier les HP pour activer/désactiver la logique de tir (ForceHitModule.Enabled)
                ...
                -- Vérifier la distance entre le joueur local et la cible (pour la logique de tir uniquement)
                local localCharacter = LocalPlayer.Character
                local localPosition = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart") and localCharacter.HumanoidRootPart.Position
                if localPosition and hitPart then
                    ...
                    if distance > 200 then
                        if ForceHitModule.Enabled then
                            ForceHitModule.Enabled = false
                            print("ForceHit: Désactivé (distance > 200m)")
                        end
                    elseif distance <= 200 then
                        if not ForceHitModule.Enabled and health > 2.5 then
                            ForceHitModule.Enabled = true
                            print("ForceHit: Activé (distance <= 200m)")
                        end
                    end

                    -- Vérifier si la cible est derrière un mur (pour la logique de tir uniquement)
                    ...
                    if ForceHitModule.WallCheckEnabled and IsTargetBehindWall(localPosition, targetPosition) then
                        if ForceHitModule.Enabled then
                            ForceHitModule.Enabled = false
                            print("ForceHit: Désactivé (cible derrière un mur)")
                        end
                    else
                        if not ForceHitModule.Enabled and health > 2.5 and distance <= 200 then
                            ForceHitModule.Enabled = true
                            print("ForceHit: Activé (cible en ligne de vue)")
                        end
                    end

                    -- Vérifier si la cible est dans le void
                    if IsTargetInVoid(targetPosition) then
                        print("ForceHit: Cible dans le void, désactivation complète")
                        ForceHitModule.Enabled = false
    ...
    -- Mettre à jour le Highlight (indépendamment de ForceHitModule.Enabled)
    ...
    -- Mettre à jour le Tracer (indépendamment de ForceHitModule.Enabled)
    UpdateTracer()
end
...
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    if not ForceHitModule.TargetStrafeEnabled or not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then return end
