...
local ForceHitModule = {
    ...
    WhitelistedPlayers = {}, -- Table to store whitelisted players
}
...
-- Whitelist Functions
function ForceHitModule:ToggleWhitelist(player)
    if self.WhitelistedPlayers[player] then
        self.WhitelistedPlayers[player] = nil
        print(player.Name .. " removed from whitelist")
    else
        self.WhitelistedPlayers[player] = true
        print(player.Name .. " added to whitelist")
    end
    -- If the current target is whitelisted, deselect them
    if self.TargetPlayer and self:IsWhitelisted(self.TargetPlayer) then
        self.TargetPlayer = nil
        self.SelectedTarget = nil
        self:UpdateTargetInfoUI()
    end
end

function ForceHitModule:IsWhitelisted(player)
    return self.WhitelistedPlayers[player] == true
end
...
local function GetClosestPlayer()
    ...
    for _, Player in pairs(Players:GetPlayers()) do
        if Player ~= LocalPlayer and not ForceHitModule:IsWhitelisted(Player) then
...
ForceHitModule.Connections["AutoStomp"] = RunService.Stepped:Connect(function(time, step)
    ...
    if ForceHitModule.TargetPlayer and not ForceHitModule:IsWhitelisted(ForceHitModule.TargetPlayer) then
