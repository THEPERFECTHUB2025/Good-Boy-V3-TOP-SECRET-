...
local function MonitorTargetRespawn()
    ...
    -- Disconnect any previous CharacterAdded connection to avoid duplicates
    ...
    ForceHitModule.Connections["TargetRespawnMonitor"] = player.CharacterAdded:Connect(function(newCharacter)
        ...
        local hitPart = newCharacter:WaitForChild(ForceHitModule.HitPart, 5) -- Wait up to 5 seconds for the hit part
        local humanoid = newCharacter:WaitForChild("Humanoid", 5)
        local forceField = newCharacter:FindFirstChildOfClass("ForceField")

        if hitPart and humanoid and humanoid.Health > 0 and not forceField then
            ForceHitModule.SelectedTarget = hitPart
            print("[TargetRespawnMonitor] Target respawned, updated SelectedTarget:", player.Name)
        else
            print("[TargetRespawnMonitor] Target respawned but invalid (no HitPart, dead, or has ForceField):", player.Name)
        end
    end)
end
...
local function UpdateTargetAndHighlight()
    ...
    -- If no target is set, select a new one based on mouse position
    if not ForceHitModule.TargetPlayer then
        print("[UpdateTargetAndHighlight] No target set, selecting new target")
        local ClosestPart, ClosestCharacter, ClosestPlayer = GetClosestPlayer()
        ForceHitModule.SelectedTarget = ClosestPart
        ForceHitModule.TargetPlayer = ClosestPlayer
        if ForceHitModule.TargetPlayer then
            print("[UpdateTargetAndHighlight] New target selected:", ForceHitModule.TargetPlayer.Name)
        end
    end
    ...
    if ForceHitModule.TargetPlayer then
        -- Check if the current target is whitelisted
        if IsPlayerWhitelisted(ForceHitModule.TargetPlayer) then
            print("[UpdateTargetAndHighlight] Target is whitelisted, clearing target:", ForceHitModule.TargetPlayer.Name)
            ForceHitModule.TargetPlayer = nil
            ForceHitModule.SelectedTarget = nil
            return
        end
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                ForceHitModule.SelectedTarget = nil -- Clear SelectedTarget but keep TargetPlayer
            end
        else
...
ForceHitModule.Connections["KillAll"] = RunService.Heartbeat:Connect(function()
    ...
    -- If we already have a target, use it; otherwise, find a new one
    if not ForceHitModule.TargetPlayer then
        ...
        ForceHitModule.TargetPlayer = closestPlayer
        ForceHitModule.SelectedTarget = closestPart
        MonitorTargetRespawn() -- Monitor the new target's respawn
    end
