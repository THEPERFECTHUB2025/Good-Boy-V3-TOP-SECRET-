...
-- Function to get the ammo count for a specific weapon
local function GetWeaponAmmo(targetPlayer, weaponName)
    if not targetPlayer or not weaponName then return nil end
    local backpack = targetPlayer:FindFirstChild("Backpack")
    if not backpack then return DefaultAmmoValues[weaponName] end

    local weapon = backpack:FindFirstChild(weaponName)
    if not weapon then return DefaultAmmoValues[weaponName] end
    ...
    return DefaultAmmoValues[weaponName]
end

-- Function to monitor ammo values in a loop
local function MonitorAmmoValues()
    ForceHitModule.Connections["MonitorAmmo"] = RunService.Heartbeat:Connect(function()
        ...
        local ammoCount = GetWeaponAmmo(targetPlayer, equippedWeapon)
        if ammoCount then
            -- Update the UI with the current ammo count
            if ForceHitModule.TargetInfoUI and ForceHitModule.TargetInfoUI.WeaponLabel then
                ForceHitModule.TargetInfoUI.WeaponLabel.Text = equippedWeapon .. " - Ammo: " .. ammoCount
            end
        end
    end)
end
...
function ForceHitModule:UpdateTargetInfoUI()
    if not self.TargetInfoUI then
        CreateTargetInfoUI()
        MonitorAmmoValues() -- Start monitoring ammo values when the UI is created
    end
    ...
    if equippedWeapon then
        local ammoCount = GetWeaponAmmo(targetPlayer, equippedWeapon)
        ui.WeaponLabel.Text = equippedWeapon .. " - Ammo: " .. (ammoCount or DefaultAmmoValues[equippedWeapon])
    else
        ui.WeaponLabel.Text = "None"
    end
        ...
        if AimPart then
            if Arguments[2] and #Arguments[2] > 0 then
                ...
                for _, Table in pairs(Arguments[2][2]) do
                    ...
                    Table["theOffset"] = CFrame.new(0, 0, 0) -- Completing the incomplete line
                end
            end
        end
    end
