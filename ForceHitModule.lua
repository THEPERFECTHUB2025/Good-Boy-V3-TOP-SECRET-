...
local ForceHitModule = {
    ...
    WhitelistedPlayers = {}, -- Liste des joueurs whitelistés
    ...
    StrafeAngle = 0 -- Angle pour le mouvement de strafe
}
...
local function GetClosestPlayerByDistance(minHealth, maxDistance)
    ...
    for _, Player in pairs(Players:GetPlayers()) do
        if Player ~= LocalPlayer then
            -- Ignorer les joueurs whitelistés
            if table.find(ForceHitModule.WhitelistedPlayers, Player.Name) then
                continue
            end
            ...
            if Character and Character:FindFirstChild("Humanoid") and Character.Humanoid.Health > 0 then
                ...
                if Part and not ForceField then
                    ...
                    if Humanoid and (not minHealth or Humanoid.Health >= minHealth) then
                        ...
                        -- Vérifier la limite de distance maximale (1000 studs)
                        if maxDistance and Distance > maxDistance then
                            continue
                        end
                        -- Ignorer les joueurs avec 5 HP ou moins
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if ForceHitModule.AutoTargetAll and health <= 0.5 then
                    ...
                    local newPart, newCharacter, newPlayer = GetClosestPlayerByDistance(nil, 1000)
                    if newPlayer then
...
-- Logique pour Kill All (téléportation, strafe et activation de ForceHit)
ForceHitModule.Connections["KillAll"] = RunService.Heartbeat:Connect(function(deltaTime)
    if not ForceHitModule.KillAllEnabled then return end

    -- Vérifier si la cible actuelle est morte ou invalide
    if ForceHitModule.TargetPlayer then
        local character = ForceHitModule.TargetPlayer.Character
        local humanoid = character and character:FindFirstChild("Humanoid")
        if not character or not humanoid or humanoid.Health <= 0 then
            print("Kill All: Cible " .. (ForceHitModule.TargetPlayer.Name or "inconnue") .. " est morte, passage à la prochaine cible")
            ForceHitModule.TargetPlayer = nil
            ForceHitModule.SelectedTarget = nil
        end
    end

    -- Si aucune cible n'est sélectionnée, en trouver une nouvelle
    if not ForceHitModule.TargetPlayer then
        local closestPart, closestCharacter, closestPlayer = GetClosestPlayerByDistance(2.5, 1000)
        if not closestPlayer then
            print("Kill All: Aucune cible avec plus de 2.5 HP trouvée dans un rayon de 1000 studs")
            return
        end
    ...
    -- Effectuer le mouvement de strafe autour de la cible
    local localCharacter = LocalPlayer.Character
    local localRootPart = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart")
    local targetCharacter = ForceHitModule.TargetPlayer and ForceHitModule.TargetPlayer.Character
    ...
    if localRootPart and targetRootPart then
        -- Mettre à jour l'angle de strafe (vitesse de rotation : 360 degrés en 2 secondes)
        ForceHitModule.StrafeAngle = ForceHitModule.StrafeAngle + (math.pi * 2 * deltaTime / 2) -- 360 degrés en 2 secondes
        if ForceHitModule.StrafeAngle > math.pi * 2 then
            ForceHitModule.StrafeAngle = ForceHitModule.StrafeAngle - math.pi * 2
        end

        -- Calculer la position autour de la cible (rayon de 5 studs)
        local radius = 5
        local offsetX = math.cos(ForceHitModule.StrafeAngle) * radius
        local offsetZ = math.sin(ForceHitModule.StrafeAngle) * radius
        local strafePosition = targetRootPart.Position + Vector3.new(offsetX, 0, offsetZ)

        -- Téléporter le joueur local à la position de strafe
        localRootPart.CFrame = CFrame.new(strafePosition, targetRootPart.Position) -- Regarder vers la cible
        print("Kill All: Strafe autour de " .. ForceHitModule.TargetPlayer.Name .. " (angle: " .. math.deg(ForceHitModule.StrafeAngle) .. " degrés)")
    end
end)
...
function ForceHitModule:SetKillAll(value)
    ...
    if value then
        self.StrafeAngle = 0 -- Réinitialiser l'angle de strafe
...
function ForceHitModule:AddToWhitelist(playerName)
    if not table.find(self.WhitelistedPlayers, playerName) then
        table.insert(self.WhitelistedPlayers, playerName)
        print("Joueur ajouté à la whitelist:", playerName)
    end
end

function ForceHitModule:RemoveFromWhitelist(playerName)
    local index = table.find(self.WhitelistedPlayers, playerName)
    if index then
        table.remove(self.WhitelistedPlayers, index)
        print("Joueur retiré de la whitelist:", playerName)
    end
end

function ForceHitModule:GetWhitelistedPlayers()
