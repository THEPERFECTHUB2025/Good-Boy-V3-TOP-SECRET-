...
local function UpdateTargetInfoUI()
    ...
    if not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then
        if ui.Frame.Visible then
            ...
            tween.Completed:Connect(function()
                ...
                ui.Frame.Size = UDim2.new(0, 300, 0, 150)
    ...
    if health <= maxHealth * 0.25 and ui.Frame.Visible then
        ...
        local tween1 = TweenService:Create(ui.Frame, tweenInfo, {Size = UDim2.new(0, 310, 0, 155)})
        local tween2 = TweenService:Create(ui.Frame, tweenInfo, {Size = UDim2.new(0, 300, 0, 150)})
        tween1:Play()
    ...
    if not ui.Frame.Visible then
        ...
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 0.5, Size = UDim2.new(0, 300, 0, 150)})
        tween:Play()
        tween.Completed:Connect(function()
            particleFrame:Destroy()
        end)
...
-- Surveillance des changements de santé pour jouer le son de hit
...
function ForceHitModule:Disable()
    ...
    -- Réinitialiser la dernière santé pour le son
    lastHealth = nil
end
