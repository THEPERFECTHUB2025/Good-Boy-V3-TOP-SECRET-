...
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    -- Ensure the player faces the target while strafing
    local lookAtCFrame = CFrame.new(newPosition, targetRootPart.Position)
    localRootPart.CFrame = CFrame.new(newPosition, lookAtCFrame.Position)
end)

ForceHitModule.Connections["AutoStomp"] = RunService.Heartbeat:Connect(function()
    if not ForceHitModule.AutoStompEnabled or not ForceHitModule.TargetPlayer then return end
...
ForceHitModule.Connections["UpdateTarget"] = RunService.Heartbeat:Connect(function()
