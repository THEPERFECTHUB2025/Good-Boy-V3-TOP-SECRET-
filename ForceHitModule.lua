...
local TweenService = game:GetService("TweenService") -- Ajouté pour les animations
...
-- Créer l'UI pour les informations sur la cible (design futuriste)
local function CreateTargetInfoUI()
    ...
    frame.BackgroundTransparency = 0.3 -- Semi-transparent (effet de verre)
    ...
    -- Ajouter un effet de glow (ombre portée)
    local shadow = Instance.new("UIStroke")
    shadow.Thickness = 4
    ...
    iconFrame.Position = UDim2.new(0, 10, 0, 45)
    ...
    nameLabel.Position = UDim2.new(0, 70, 0, 45)
    ...
    healthLabel.Size = UDim2.new(0, 140, 0, 25)
    healthLabel.Position = UDim2.new(0, 70, 0, 70)
...
local function UpdateTargetInfoUI()
    ...
    if not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then
        if ui.Frame.Visible then
            ...
            local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 1, Position = UDim2.new(0, -220, 0, 10)})
            tween:Play()
            tween.Completed:Connect(function()
                ui.Frame.Visible = false
                ui.Frame.BackgroundTransparency = 0.3
    ...
    if not ui.Frame.Visible then
        ...
        ui.Frame.Position = UDim2.new(0, -220, 0, 10)
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 0.3, Position = UDim2.new(0, 10, 0, 10)})
        tween:Play()
    end
end
...
local function UpdateTargetAndHighlight()
    ...
    if not ForceHitModule.ManuallyEnabled then
        ...
        Highlight.Enabled = false
        ForceHit10 = Instance.new("BillboardGui")
