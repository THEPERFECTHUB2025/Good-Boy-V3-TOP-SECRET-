...
local function CreateTargetInfoUI()
    ...
    -- Ajouter un gradient de fond vibrant
    ...
    local function flickerBorder()
        while true do
            ...
            local tween1 = TweenService:Create(stroke, tweenInfo, {Transparency = 0.4})
    ...
    healthLabel.Size = UDim2.new(0, 180, 0, 25)
...
local function UpdateTargetInfoUI()
    ...
    if not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then
        if ui.Frame.Visible then
            ...
            local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 1, Position = UDim2.new(0, -260, 0, 10)})
            tween:Play()
            tween.Completed:Connect(function()
    ...
    if not ui.Frame.Visible then
        ...
        ui.Frame.Position = UDim2.new(0, -260, 0, 10)
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
...
-- Fonction pour obtenir la position 3D de la souris (corrigée pour un alignement précis)
local function GetMousePositionInWorld()
    local mousePos = UserInputService:GetMouseLocation()
    -- Ajuster pour prendre en compte les barres de titre ou décalages d'interface
    local viewportSize = Camera.ViewportSize
    local adjustedMousePos = Vector2.new(mousePos.X, mousePos.Y)

    -- Créer un rayon à partir de la position ajustée de la souris
    local ray = Camera:ScreenPointToRay(adjustedMousePos.X, adjustedMousePos.Y)
    local raycastParams = RaycastParams.new()
    ...
    if raycastResult then
        -- Si un objet est touché, utiliser la position d'impact
        return raycastResult.Position
    else
        -- Si aucun objet n'est touché, calculer une position proche de la caméra
        -- Ajuster la distance pour que le point de départ soit proche du curseur
        local distance = 5 -- Réduire la distance pour un meilleur alignement visuel
        local mouseWorldPos = ray.Origin + ray.Direction * distance

        -- Ajouter un offset pour compenser le décalage horizontal observé
        local offset = Vector3.new(0.5, 0, 0) -- Ajuste cet offset selon le décalage observé (positif pour déplacer à droite)
        return mouseWorldPos + offset
    end
end
