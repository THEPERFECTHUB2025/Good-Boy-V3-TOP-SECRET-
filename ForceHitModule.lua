...
ForceHitModule.Connections["KillAll"] = RunService.Heartbeat:Connect(function()
    ...
    local closestPart, closestCharacter, closestPlayer = nil, nil, nil
    local closestDistance = math.huge

    for _, Player in pairs(Players:GetPlayers()) do
        if Player ~= LocalPlayer then
            local Character = Player.Character
            if Character and Character:FindFirstChild("Humanoid") and Character.Humanoid.Health >= 20 then -- Check for 20+ HP
                local Part = Character:FindFirstChild(ForceHitModule.HitPart)
                local ForceField = Character:FindFirstChildOfClass("ForceField")
                if Part and not ForceField then
                    ...
                    if targetRootPart then
                        local distance = (localRootPart.Position - targetRootPart.Position).Magnitude
                        if distance <= maxRadius and distance < closestDistance then -- Within 500 meters
                            closestDistance = distance
                            closestPart = Part
                            closestCharacter = Character
                            closestPlayer = Player
