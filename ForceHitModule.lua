...
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if IsTargetInVoid(targetRootPart.Position) then
        print("[TargetStrafe] Target in void, skipping")
        return
    end

    -- Increase radius to a reasonable value (e.g., 10 studs) for noticeable circling
    local radius = 10 -- Adjust this value as needed (10 studs is a good starting point)
    ...
    -- Adjust the Y position to match the target's height, but do it in one step
    ...
    print("[TargetStrafe] Circling target at position: " .. tostring(newPosition) .. ", distance: " .. math.floor(distance) .. " studs")
end)
