...
-- Valeurs par défaut des munitions pour chaque arme
...
local function GetEquippedWeaponAmmo(targetPlayer, equippedWeapon)
    ...
    -- Si l'arme n'est pas équipée, vérifier dans le sac à dos
    ...
    -- Retourner la valeur par défaut si aucune munition n'est trouvée
...
function ForceHitModule:UpdateTargetInfoUI()
    ...
    if not humanoid or not targetRootPart or not localRootPart then
        if ui.Frame.Visible then
            ...
            local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local tween = TweenService:Create(ui.Frame, tween

            Info.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 1, Size = UDim2.new(0, 0, 0, 0)})
            tween:Play()
            tween.Completed:Connect(function()
                ui.Frame.Visible = false
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if localPosition and hitPart then
                    ...
                    -- Suppression de la vérification de distance pour permettre au Target Strafe de fonctionner à n'importe quelle distance
                    -- On garde uniquement la vérification du mur et du vide
...
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    local distance = (localRootPart.Position - targetRootPart.Position).Magnitude
    if distance > 100000 then -- Limite maximale de 100 000 studs pour éviter des bugs
        return
    end
...
function ForceHitModule:Enable()
    ...
    -- Démarrer la surveillance des munitions
    MonitorTargetAmmo()
end
...
function ForceHitModule:SetTargetStrafe(value)
    ...
    if value then
        -- Sauvegarder la position actuelle du joueur quand Target Strafe est activé
        local localCharacter = LocalPlayer.Character
        local localRootPart = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart")
        ...
        -- Téléporter le joueur à la position sauvegardée quand Target Strafe est désactivé
