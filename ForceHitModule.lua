...
local function SetupUIMobileSupport()
    ...
    if ForceHitModule.TargetInfoUI then
        ...
        if UserInputService.TouchEnabled then
            ...
            uiWidth = math.min(200, viewportSize.X * 0.5) -- 50% de la largeur max, limité à 200
            uiHeight = math.min(120, viewportSize.Y * 0.3) -- 30% de la hauteur max, limité à 120
            uiPosition = UDim2.new(0, 5, 0.5, -uiHeight / 2) -- Côté gauche, centré verticalement
        else
            -- Ajustements pour PC
            uiWidth = math.min(300, viewportSize.X * 0.3) -- 30% de la largeur max, limité à 300
            uiHeight = math.min(180, viewportSize.Y * 0.25) -- 25% de la hauteur max, limité à 180
            uiPosition = UDim2.new(0, 10, 0.5, -uiHeight / 2) -- Côté gauche, centré verticalement
        end
