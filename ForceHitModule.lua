...
local function CreateTargetInfoUI()
    ...
    gradient.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(0.66, Color3.fromRGB(0, 255, 200)),
    ...
    local function flickerBorder()
        while true do
            ...
            local tween1 = TweenService:Create(stroke, tweenInfo, {Transparency = 0.15})
    ...
    -- Ajouter un effet de distorsion (variation subtile de taille et position)
    local function distortionEffect()
        while true do
            ...
            local tween1 = TweenService:Create(frame, tweenInfo, {
                ...
                Size = UDim2.new(0, 320 + sizeOffset, 0, 160 + sizeOffset)
            })
            tween1:Play()
            tween1.Completed:Wait()
        end
    ...
    -- Ajouter un effet de grille holographique (lignes subtiles en arrière-plan)
    ...
    local function createGridLine(isHorizontal)
        ...
        local tween
        if isHorizontal then
            tween = TweenService:Create(line, tweenInfo, {Position = UDim2.new(0, 0, math.random(), 0)})
        else
            tween = TweenService:Create(line, tweenInfo, {Position = UDim2.new(math.random(), 0, 0, 0)})
        end
        tween:Play()
    ...
    local function spawnGridLines()
        while true do
            createGridLine(true) -- Ligne horizontale
            createGridLine(false) -- Ligne verticale
            wait(0.5)
        end
    end
    spawn(spawnGridLines)
    ...
    -- Ajouter un effet de "bruit numérique" (particules statiques)
    local noiseFrame = Instance.new("Frame")
    noiseFrame.Size = UDim2.new(1, 0, 1, 0)
    noiseFrame.BackgroundTransparency = 1
    noiseFrame.Parent = frame

    local function createNoiseParticle()
        local noise = Instance.new("Frame")
        noise.Size = UDim2.new(0, 2, 0, 2)
        noise.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        noise.BackgroundTransparency = 0.5
        noise.Position = UDim2.new(math.random(), 0, math.random(), 0)
        noise.Parent = noiseFrame

        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Linear)
        local tween = TweenService:Create(noise, tweenInfo, {BackgroundTransparency = 1})
        tween:Play()
        tween.Completed:Connect(function()
            noise:Destroy()
        end)
    ...
    local function spawnNoiseParticles()
        while true do
            createNoiseParticle()
            wait(0.05)
        end
    end
    spawn(spawnNoiseParticles)
    ...
    -- Ajouter un effet de pulsation sur le titre
    local function pulseTitle()
        while true do
            local tweenInfo = TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
            local tween1 = TweenService:Create(titleLabel, tweenInfo, {TextTransparency = 0.3})
            local tween2 = TweenService:Create(titleLabel, tweenInfo, {TextTransparency = 0})
            tween1:Play()
            tween1.Completed:Wait()
            tween2:Play()
            tween2.Completed:Wait()
    ...
    local function pulseIcon()
        while true do
            local tweenInfo = TweenInfo.new(0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
            local tween1 = TweenService:Create(iconFrame, tweenInfo, {BackgroundTransparency = 0.8, Size = UDim2.new(0, 54, 0, 54)})
            local tween2 = TweenService:Create(iconFrame, tweenInfo, {BackgroundTransparency = 0.6, Size = UDim2.new(0, 50, 0, 50)})
            tween1:Play()
            tween1.Completed:Wait()
            tween2:Play()
            tween2.Completed:Wait()
    ...
    -- Ajouter un réticule de verrouillage autour de l'icône
    local lockFrame = Instance.new("Frame")
    lockFrame.Size = UDim2.new(0, 60, 0, 60)
    lockFrame.Position = UDim2.new(0, 5, 0, 45)
    lockFrame.BackgroundTransparency = 1
    lockFrame.Parent = frame

    local function createLockSegment(startAngle, endAngle)
        local segment = Instance.new("Frame")
        segment.Size = UDim2.new(0, 2, 0, 10)
        segment.Position = UDim2.new(0.5, -1, 0.5, -5)
        segment.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
        segment.BackgroundTransparency = 0.3
        segment.Rotation = startAngle
        segment.Parent = lockFrame

        local tweenInfo = TweenInfo.new(5, Enum.EasingStyle.Linear)
        local tween = TweenService:Create(segment, tweenInfo, {Rotation = endAngle})
        tween:Play()
    end

    local function spawnLockSegments()
        for i = 1, 4 do
            local startAngle = (i - 1) * 90
            createLockSegment(startAngle, startAngle + 360)
        end
    end
    spawn(spawnLockSegments)
    ...
    -- Ajouter un effet de pulsation sur le nom
    local function pulseName()
        while true do
            local tweenInfo = TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
            local tween1 = TweenService:Create(nameLabel, tweenInfo, {TextTransparency = 0.3})
            local tween2 = TweenService:Create(nameLabel, tweenInfo, {TextTransparency = 0})
            tween1:Play()
            tween1.Completed:Wait()
            tween2:Play()
            tween2.Completed:Wait()
    ...
    healthLabel.Font = Enum.Font.SciFi
    ...
    -- Ajouter un effet de pulsation sur la santé
    local function pulseHealth()
        while true do
            local tweenInfo = TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
            local tween1 = TweenService:Create(healthLabel, tweenInfo, {TextTransparency = 0.3})
            local tween2 = TweenService:Create(healthLabel, tweenInfo, {TextTransparency = 0})
            tween1:Play()
            tween1.Completed:Wait()
            tween2:Play()
            tween2.Completed:Wait()
    ...
    distanceLabel.Position = UDim2.new(0, 200, 0, 75)
    ...
    local function glitchDistanceEffect()
        while true do
            ...
            distanceLabel.Position = UDim2.new(0, 200 + math.random(-1, 1), 0, 75)
            wait(0.05)
    ...
    -- Ajouter un effet de pulsation sur la distance
    local function pulseDistance()
        while true do
            local tweenInfo = TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
            local tween1 = TweenService:Create(distanceLabel, tweenInfo, {TextTransparency = 0.3})
            local tween2 = TweenService:Create(distanceLabel, tweenInfo, {TextTransparency = 0})
            tween1:Play()
            tween1.Completed:Wait()
            tween2:Play()
            tween2.Completed:Wait()
...
local function UpdateTargetInfoUI()
    ...
    if not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then
        if ui.Frame.Visible then
            -- Animation de disparition avec effet de "désassemblage"
            local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
            local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 1, Size = UDim2.new(0, 0, 0, 160)})
            tween:Play()
            tween.Completed:Connect(function()
                ui.Frame.Visible = false
                ui.Frame.BackgroundTransparency = 0.5
                ui.Frame.Size = UDim2.new(0, 320, 0, 160)
    ...
    -- Ajouter un effet de "surcharge" si la cible est très proche (≤ 10 studs)
    if distance <= 10 and ui.Frame.Visible then
        local tweenInfo = TweenInfo.new(0.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
        local tween1 = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 0.3})
        local tween2 = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 0.5})
        tween1:Play()
        tween1.Completed:Wait()
        tween2:Play()
    end
    ...
    if not ui.Frame.Visible then
        ...
        ui.Frame.Size = UDim2.new(0, 0, 0, 160)
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 0.5, Size = UDim2.new(0, 320, 0, 160)})
        tween:Play()
    end
end
