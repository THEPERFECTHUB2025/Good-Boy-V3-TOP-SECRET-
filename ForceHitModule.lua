...
local ForceHitModule = {
    ...
    SavedPosition = nil,
    LastAmmoValue = nil,
    -- Ajout pour le keybind
    TargetStrafeKeybind = Enum.KeyCode.G, -- Touche par défaut pour Target Strafe
    -- Ajout pour la whitelist
    WhitelistedPlayers = {}, -- Liste des joueurs whitelistés
    WhitelistUI = nil, -- UI pour le dropdown de la whitelist
}
...
-- Nouvelle fonction : Créer l'UI pour la Whitelist
local function CreateWhitelistUI()
    ...
    titleLabel.Position = UDim2.new(0, 0, 0, 0)
    titleLabel.BackgroundTransparency = 1
    titleLabel.Text = "Whitelist Players"
    ...
    local function UpdateWhitelistUI()
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer then
                ...
                button.BackgroundColor3 = table.find(ForceHitModule.WhitelistedPlayers, player) and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(50, 50, 50)
                ...
                button.MouseButton1Click:Connect(function()
                    local index = table.find(ForceHitModule.WhitelistedPlayers, player)
                    if index then
                        table.remove(ForceHitModule.WhitelistedPlayers, index)
                        button.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
                    else
                        table.insert(ForceHitModule.WhitelistedPlayers, player)
        ...
        -- Ajuster la taille du ScrollingFrame
        scrollingFrame.CanvasSize = UDim2.new(0, 0, 0, uiListLayout.AbsoluteContentSize.Y)
    end

    -- Mettre à jour la liste lorsque des joueurs rejoignent ou quittent
    ...
    ForceHitModule.WhitelistUI = screenGui
end
...
local function GetClosestPlayer()
    ...
    for _, Player in pairs(Players:GetPlayers()) do
        if Player ~= LocalPlayer and not table.find(ForceHitModule.WhitelistedPlayers, Player) then
...
local function UpdateTargetAndHighlight()
    ...
    if ForceHitModule.TargetPlayer then
        -- Vérifier si le joueur cible est dans la whitelist
        if table.find(ForceHitModule.WhitelistedPlayers, ForceHitModule.TargetPlayer) then
...
ForceHitModule.Connections["KillAll"] = RunService.Heartbeat:Connect(function()
    ...
    local closestPart, closestCharacter, closestPlayer = GetClosestPlayerByDistance(20, maxRadius, false)
...
ForceHitModule.Connections["AutoStomp"] = RunService.Stepped:Connect(function(time, step)
    ...
    -- Ne pas exécuter AutoStomp sur les joueurs whitelistés
    if ForceHitModule.TargetPlayer and table.find(ForceHitModule.WhitelistedPlayers, ForceHitModule.TargetPlayer) then return end
...
-- Nouvelle fonction : Configurer le keybind pour Target Strafe
local function SetupTargetStrafeKeybind()
    ForceHitModule.Connections["TargetStrafeKeybind"] = UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
        if gameProcessedEvent then return end -- Ignorer si l'entrée est utilisée par le jeu (ex: chat)

        if input.KeyCode == ForceHitModule.TargetStrafeKeybind then
            ForceHitModule:SetTargetStrafe(not ForceHitModule.TargetStrafeEnabled)
        end
    end)
end
...
function ForceHitModule:SetHitPart(part)
    self.HitPart = part
end

function ForceHitModule:GetHitPart()
    return self.HitPart
end

function ForceHitModule:SetBlankShots(value)
    self.BlankShots = value
end

function ForceHitModule:GetBlankShots()
    return self.BlankShots
end
...
function ForceHitModule:Cleanup()
    ...
    if self.WhitelistUI then
        self.WhitelistUI:Destroy()
        self.WhitelistUI = nil
    end
