        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if localPosition and hitPart then
                    ...
                    -- Only disable shooting if the target is behind a wall and WallCheck is enabled
                    if ForceHitModule.WallCheckEnabled and IsTargetBehindWall(localPosition, targetPosition) then
                        if ForceHitModule.Enabled then
                            ForceHitModule.Enabled = false
                        end
                    else
                        -- Allow the force hit to remain active regardless of distance or void
...
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if not humanoid or humanoid.Health < 20 then return end
    ...
    if IsTargetInVoid(targetRootPart.Position) then return end
...
local function UpdateTargetAndHighlight()
    if not ForceHitModule.ManuallyEnabled then
        print("[UpdateTargetAndHighlight] ManuallyEnabled is false, disabling")
        ForceHitModule.Enabled = false
        if ForceHitModule.Highlight then
            ForceHitModule.Highlight.Enabled = false
        end
    ...
    if ForceHitModule.TargetPlayer then
        -- Check if the current target is whitelisted
        if IsPlayerWhitelisted(ForceHitModule.TargetPlayer) then
            print("[UpdateTargetAndHighlight] Target is whitelisted, finding new target")
            ForceHitModule.TargetPlayer = nil
            ForceHitModule.SelectedTarget = nil
            -- Find a new non-whitelisted target
            local ClosestPart, ClosestCharacter, ClosestPlayer = GetClosestPlayer()
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if health >= 0.5 and health <= 20 then
                    if ForceHitModule.Enabled then
                        print("[UpdateTargetAndHighlight] Health between 0.5 and 20, disabling: " .. health)
                        ForceHitModule.Enabled = false
                    end
                elseif health > 20 then
                    if not ForceHitModule.Enabled then
                        print("[UpdateTargetAndHighlight] Health > 20, enabling: " .. health)
                        ForceHitModule.Enabled = true
                    end
                end
                ...
                if ForceHitModule.AutoTargetAll then
                    ...
                    if currentTime - ForceHitModule.LastTargetSwitchTime >= 0.5 then
                        ...
                        local newPart, newCharacter, newPlayer = GetClosestPlayerByDistance(20, maxRadius, true)
                        if newPlayer and newPlayer ~= ForceHitModule.TargetPlayer then
                            print("[UpdateTargetAndHighlight] AutoTargetAll switching to new target: " .. newPlayer.Name)
                            ForceHitModule.TargetPlayer = newPlayer
                            ForceHitModule.SelectedTarget = newPart
                        end
                    end
                ...
                if localPosition and hitPart then
                    ...
                    if ForceHitModule.WallCheckEnabled and IsTargetBehindWall(localPosition, targetPosition) then
                        if ForceHitModule.Enabled then
                            print("[UpdateTargetAndHighlight] Target behind wall, disabling")
                            ForceHitModule.Enabled = false
                        end
                    else
                        -- Allow the force hit to remain active regardless of distance or void
                        if not ForceHitModule.Enabled and health > 20 then
                            print("[UpdateTargetAndHighlight] No wall obstruction, health > 20, enabling")
                            ForceHitModule.Enabled = true
                        end
                    end
                    print("[UpdateTargetAndHighlight] Distance to target: " .. math.floor(distance) .. " studs, Enabled: " .. tostring(ForceHitModule.Enabled))
                else
                    print("[UpdateTargetAndHighlight] Local position or hitPart not found")
                end
            else
                print("[UpdateTargetAndHighlight] HitPart, Humanoid, or no ForceField check failed")
                ForceHitModule.SelectedTarget = nil
            end
        else
            print("[UpdateTargetAndHighlight] Target character not found")
            ForceHitModule.SelectedTarget = nil
        end
    else
        print("[UpdateTargetAndHighlight] No TargetPlayer set")
    end
...
OriginalNameCall = hookmetamethod(game, "__namecall", function(Object, ...)
    ...
    if not ForceHitModule.Enabled then
        print("[Namecall] ForceHitModule not enabled")
    ...
    if NameCallMethod == "InvokeServer" and Object.Name == "MainFunction" and #Arguments > 0 and Arguments[1] == "GunCheck" then
        print("[Namecall] Blocking GunCheck")
        return nil
    end
    ...
    if NameCallMethod == "FireServer" and Object.Name == "MainEvent" and #Arguments > 0 and Arguments[1] == "Shoot" then
        local AimPart = ForceHitModule.SelectedTarget
        if not AimPart then
            print("[Namecall] No AimPart (SelectedTarget) found")
            return OriginalNameCall(Object, ...)
        end
        ...
        if not localPosition then
            print("[Namecall] Local position (HumanoidRootPart) not found")
            return OriginalNameCall(Object, ...)
        end

        local distance = (localPosition - targetPosition).Magnitude
        if distance > 714.2 then -- 200 meters ≈ 714.2 studs
            print("[Namecall] Target too far: " .. math.floor(distance) .. " studs")
            return OriginalNameCall(Object, ...)
        end

        -- Check if the target has a ForceField
        local AimChar = AimPart.Parent
        if not AimChar then
            print("[Namecall] No AimChar (parent of AimPart) found")
            return OriginalNameCall(Object, ...)
        end

        local ForceField = AimChar:FindFirstChildOfClass("ForceField")
        if ForceField then
            print("[Namecall] Target has a ForceField, skipping")
            return OriginalNameCall(Object, ...)
        end

        -- Check if the target is whitelisted
        local targetPlayer = Players:GetPlayerFromCharacter(AimChar)
        if targetPlayer and IsPlayerWhitelisted(targetPlayer) then
            print("[Namecall] Target is whitelisted, skipping")
            return OriginalNameCall(Object, ...)
        end

        -- All checks passed, proceed to shoot
        print("[Namecall] Shooting at target at distance: " .. math.floor(distance) .. " studs")
...
ForceHitModule.Connections["BlankShots"] = RunService.Heartbeat:Connect(function()
    if not ForceHitModule.Enabled or not ForceHitModule.BlankShots then
        print("[BlankShots] Not enabled or BlankShots disabled")
        return
    end

    -- Check if the player has a tool in their backpack
    local HasTool = false
    for _, item in pairs(LocalPlayer.Backpack:GetChildren()) do
        if item:IsA("Tool") then
            HasTool = true
    ...
    if not HasTool then
        print("[BlankShots] No tool found in backpack")
        return
    end

    -- Check if the player has a tool equipped
    local localCharacter = LocalPlayer.Character
    local equippedTool = localCharacter and localCharacter:FindFirstChildOfClass("Tool")
    if not equippedTool then
        print("[BlankShots] No tool equipped")
        return
    end

    local AimPart = ForceHitModule.SelectedTarget
    if not AimPart then
        print("[BlankShots] No AimPart (SelectedTarget) found")
        return
    end

    local AimChar = AimPart and AimPart.Parent
    if not AimChar then
        print("[BlankShots] No AimChar (parent of AimPart) found")
        return
    end

    local targetPlayer = Players:GetPlayerFromCharacter(AimChar)
    if not targetPlayer then
        print("[BlankShots] No target player found for AimChar")
        return
    end

    if IsPlayerWhitelisted(targetPlayer) then
        print("[BlankShots] Target is whitelisted, skipping")
        return
    end

    local ForceField = AimChar:FindFirstChildOfClass("ForceField")
    if ForceField then
        print("[BlankShots] Target has a ForceField, skipping")
        return
    end
    ...
    local targetPosition = AimPart.Position
    if not localPosition then
        print("[BlankShots] Local position (HumanoidRootPart) not found")
        return
    end

    local distance = (localPosition - targetPosition).Magnitude
    if distance > 714.2 then -- 200 meters ≈ 714.2 studs
        print("[BlankShots] Target too far: " .. math.floor(distance) .. " studs")
        return
    end

    if not MainEvent then
        print("[BlankShots] MainEvent not found")
        return
    end

    -- All checks passed, proceed to shoot
    print("[BlankShots] Shooting at target: " .. targetPlayer.Name .. " at distance: " .. math.floor(distance) .. " studs")
    local args = {
        "Shoot",
        {
            {
