...
local ForceHitModule = {
    ...
    TargetInfoUI = nil,
    HitSound = nil,
    SavedPosition = nil, -- To store the player's position when Target Strafe is activated
    LastAmmoValue = nil, -- To track the last known ammo value for change detection
    Highlight = nil, -- Store Highlight in the module
}
...
ForceHitModule.Highlight = Instance.new("Highlight")
ForceHitModule.Highlight.Parent = game.CoreGui
ForceHitModule.Highlight.FillColor = Color3.fromRGB(0, 255, 0)
ForceHitModule.Highlight.OutlineColor = Color3.fromRGB(255, 255, 255)
ForceHitModule.Highlight.FillTransparency = 0.5
ForceHitModule.Highlight.OutlineTransparency = 0
...
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if not humanoid or humanoid.Health < 20 then return end -- Stop if HP is less than 20
    ...
    local distance = (localRootPart.Position - targetRootPart.Position).Magnitude
    if distance > 100000 then -- Maximum distance of 100,000 studs
        return
    end
    ...
    -- Update the local player's position to strafe around the target
    localRootPart.CFrame = CFrame.new(newPosition, targetRootPart.Position)
    -- Adjust the Y position to match the target's height, preventing vertical drift
    localRootPart.CFrame = localRootPart.CFrame + Vector3.new(0, targetRootPart.Position.Y - localRootPart.Position.Y, 0)
end)

ForceHitModule.Connections["AutoStomp"] = RunService.Heartbeat:Connect(function()
    if not ForceHitModule.AutoStompEnabled or not ForceHitModule.Enabled or not ForceHitModule.TargetPlayer then return end

    local targetCharacter = ForceHitModule.TargetPlayer.Character
    local humanoid = targetCharacter and targetCharacter:FindFirstChild("Humanoid")
    if not humanoid then return end

    -- Check if the target is knocked (health between 0.5 and 20, typically indicating a knocked state)
    if humanoid.Health >= 0.5 and humanoid.Health <= 20 then
        local localCharacter = LocalPlayer.Character
        local localRootPart = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart")
        local targetRootPart = targetCharacter and targetCharacter:FindFirstChild("HumanoidRootPart")
        ...
        if localRootPart and targetRootPart then
            local distance = (localRootPart.Position - targetRootPart.Position).Magnitude
            if distance <= 10 then -- Only stomp if within 10 studs
                -- Fire the stomp event (assuming MainEvent is used for game actions)
                if MainEvent then
...
ForceHitModule.Connections["ForceHit"] = RunService.Heartbeat:Connect(function()
    ...
    local localRootPart = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart")
    if not localHumanoid or not localRootPart or localHumanoid.Health <= 0 then return end

    local targetPart = ForceHitModule.SelectedTarget
    ...
    -- Check if the local player is holding a weapon
    local equippedWeapon = GetEquippedWeapon(localCharacter)
    if not equippedWeapon then return end

    -- Simulate a hit by firing the MainEvent (assuming this is how the game handles hits)
    if MainEvent then
        local direction = (targetPart.Position - localRootPart.Position).Unit
        local hitPosition = targetPart.Position

        -- Fire the hit event to the server
        MainEvent:FireServer("Hit", targetPart, hitPosition, direction)
        ...
        -- If BlankShots is disabled, reduce the ammo manually
        if not ForceHitModule.BlankShots then
            local tool = localCharacter:FindFirstChild(equippedWeapon)
            if tool then
...
function ForceHitModule:Enable()
    ...
    -- Reinitialize Highlight if it was destroyed
...
-- Setter methods for toggling features
function ForceHitModule:SetAutoTargetAll(enabled)
    self.AutoTargetAll = enabled
end

function ForceHitModule:SetUIMobileSupport(enabled)
    self.UIMobileSupportEnabled = enabled
    SetupUIMobileSupport()
end

function ForceHitModule:SetWallCheck(enabled)
    self.WallCheckEnabled = enabled
end

function ForceHitModule:SetKillAll(enabled)
    self.KillAllEnabled = enabled
end

function ForceHitModule:SetAutoStomp(enabled)
    self.AutoStompEnabled = enabled
end

function ForceHitModule:SetTargetStrafe(enabled)
    self.TargetStrafeEnabled = enabled
    if enabled then
        -- Save the player's position when enabling Target Strafe
        local localCharacter = LocalPlayer.Character
        local localRootPart = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart")
        if localRootPart then
            self.SavedPosition = localRootPart.Position
        end
    else
        -- Restore the player's position when disabling Target Strafe
        if self.SavedPosition then
            local localCharacter = LocalPlayer.Character
            local localRootPart = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart")
            if localRootPart then
                localRootPart.Position = self.SavedPosition
            end
            self.SavedPosition = nil
        end
        self.StrafeAngle = 0
...
function ForceHitModule:SetBlankShots(enabled)
    self.BlankShots = enabled
end

function ForceHitModule:SetHitPart(partName)
    self.HitPart = partName
end

-- Initialize the module
ForceHitModule.Connections["Update"] = RunService.RenderStepped:Connect(UpdateTargetAndHighlight)

-- Setup UI mobile support based on initial settings
SetupUIMobileSupport()
