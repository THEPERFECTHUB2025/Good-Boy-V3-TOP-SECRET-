...
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    if not ForceHitModule.TargetStrafeEnabled or not ForceHitModule.Enabled or not ForceHitModule.TargetPlayer then
        print("[TargetStrafe] Not enabled or no target player")
        return
    end
    ...
    if not humanoid or humanoid.Health < 20 then
        print("[TargetStrafe] Target humanoid not found or health < 20: " .. (humanoid and humanoid.Health or "N/A"))
        return
    end
    ...
    -- Optional: Keep the void check if you want to stop following in the void
    -- Remove this if you want to follow even in the void (since your force hit now persists in the void)
    if IsTargetInVoid(targetRootPart.Position) then
        print("[TargetStrafe] Target is in void")
        return
    end

    -- Position your character 2 studs behind the target, facing them
    local offset = Vector3.new(0, 0, 0) -- 2 studs behind the target
    local targetCFrame = targetRootPart.CFrame
    local newPosition = targetCFrame * CFrame.new(offset) -- Position behind the target

    -- Set your character's position and orient it to face the target
    localRootPart.CFrame = CFrame.new(newPosition.Position, targetRootPart.Position)

    -- Optional: Adjust the Y position to match the target's height (if on uneven terrain)
    localRootPart.CFrame = localRootPart.CFrame + Vector3.new(0, targetRootPart.Position.Y - localRootPart.Position.Y, 0)

    -- Debug print to confirm the strafe is working
    local distance = (localRootPart.Position - targetRootPart.Position).Magnitude
    print("[TargetStrafe] Following target: " .. ForceHitModule.TargetPlayer.Name .. " at distance: " .. math.floor(distance) .. " studs")
end)
