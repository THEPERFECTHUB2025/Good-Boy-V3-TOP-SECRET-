...
local ForceHitModule = {
    ...
    AutoTargetAll = false, -- Ensure this is false
...
local function UpdateTargetAndHighlight()
    if not ForceHitModule.ManuallyEnabled then
        ...
        -- Do NOT clear TargetPlayer here; keep it to lock onto the same player
        UpdateTracer()
        ForceHitModule:UpdateTargetInfoUI()
        return
    end

    -- Only select a new target if TargetPlayer is nil (i.e., no target has been set yet)
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if IsPlayerWhitelisted(ForceHitModule.TargetPlayer) then
            ...
            ForceHitModule.SelectedTarget = nil
            -- Let the next loop select a new target since this one is whitelisted
            return
        end
...
function ForceHitModule:Disable()
    ...
    self.TargetPlayer = nil -- Clear TargetPlayer when disabling
    if self.Highlight then
