...
local ForceHitModule = {
    ...
    KillAllEnabled = false, -- Nouvelle option pour Kill All
    AutoStompEnabled = false, -- Nouvelle option pour Auto Stomp
    BlankShots = true,
    HitPart = "Head",
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if health >= 0.5 and health <= 10 then -- Nouvelle plage de HP (0.5 à 2.5)
                ...
                if localPosition and hitPart then
                    ...
                    elseif distance <= 200 then
                        if not ForceHitModule.Enabled and health > 10 then
                            ForceHitModule.Enabled = true
                        end
                    end
                    ...
                    if ForceHitModule.WallCheckEnabled and IsTargetBehindWall(localPosition, targetPosition) then
                        ...
                        if not ForceHitModule.Enabled and health > 10 and distance <= 200 then
                            ForceHitModule.Enabled = true
...
ForceHitModule.Connections["KillAll"] = RunService.Heartbeat:Connect(function()
    ...
    -- Vérifier si 2 secondes se sont écoulées depuis la dernière exécution
    local currentTime = os.clock()
    if currentTime - ForceHitModule.LastKillAllTime < 0.1 then return end
