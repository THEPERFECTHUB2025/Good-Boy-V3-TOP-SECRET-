...
ForceHitModule.Connections["BlankShots"] = RunService.Heartbeat:Connect(function()
    ...
    -- Ensure the local player has a character
    local localCharacter = LocalPlayer.Character
    if not localCharacter then return end

    -- Find an equipped tool
    ...
    -- If no tool is equipped, try to equip one from the backpack
    if not equippedTool then
        for _, item in pairs(LocalPlayer.Backpack:GetChildren()) do
            if item:IsA("Tool") then
                equippedTool = item
                LocalPlayer.Character:FindFirstChildOfClass("Humanoid"):EquipTool(equippedTool)
                print("[BlankShots] Equipped tool:", equippedTool.Name)
                break
            end
        end
    end

    -- If still no tool, exit
    if not equippedTool then
        print("[BlankShots] No tool found in backpack to equip")
        return
    end

    -- Check the ammo of the equipped tool
    local ammoValue = nil
    ...
    -- If out of ammo, simulate a reload
    if ammoValue ~= nil and ammoValue <= 0 then
        print("[BlankShots] Out of ammo, reloading")
    ...
    if AimChar then
        -- Skip if the target has a force field
        local ForceField = AimChar:FindFirstChildOfClass("ForceField")
        if ForceField then return end

        -- Skip if the target is whitelisted
        local targetPlayer = Players:GetPlayerFromCharacter(AimChar)
        if targetPlayer and IsPlayerWhitelisted(targetPlayer) then return end

        if AimPart and MainEvent then
            -- Get the local player's head position for realistic shooting
            local localHead = localCharacter:FindFirstChild("Head")
            local localHeadPosition = localHead and localHead.Position or localCharacter:FindFirstChild("HumanoidRootPart").Position

            -- Calculate the direction (normal) from the local player to the target
            local targetPosition = AimPart.Position
            local direction = (targetPosition - localHeadPosition).Unit
            local normal = direction

            -- Create the arguments table with dynamic positions and normals
            local args = {
                "Shoot",
                {
                    {
            ...
            print("[BlankShots] Fired shot at target:", targetPlayer and targetPlayer.Name or "Unknown")
        end
    end
end)
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if localPosition and hitPart then
                    local distance = (localPosition - hitPart.Position).Magnitude
                    -- Toggle BlankShots based on distance
                    if distance >= 200 then
                        ForceHitModule.BlankShots = true
                        print("[UpdateTargetAndHighlight] Distance >= 200 studs, BlankShots enabled")
                    else
