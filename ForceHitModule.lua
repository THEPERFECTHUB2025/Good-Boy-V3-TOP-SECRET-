...
local function CreateTargetInfoUI()
    ...
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(0, 300, 0, 180) -- Reverted to original size since we're not adding a new label
...
-- Function to get the ammo count for the equipped weapon only
local function GetEquippedWeaponAmmo(targetPlayer, equippedWeapon)
    ...
    if not backpack then return DefaultAmmoValues[equippedWeapon] end

    local weapon = backpack:FindFirstChild(equippedWeapon)
    if not weapon then return DefaultAmmoValues[equippedWeapon] end
