...
function ForceHitModule:UpdateTargetInfoUI()
    ...
    if not getgenv().Rake.Settings.Misc.TargetInfoEnabled or not selfව:FindFirstChild("Main").ManuallyEnabled or not self.TargetPlayer then
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if localPosition and hitPart then
                    ...
                    -- La vérification de distance a été supprimée pour permettre au Target Strafe de fonctionner à n'importe quelle distance
                    -- (comme demandé dans la requête précédente)
