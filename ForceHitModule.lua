...
local ForceHitModule = {
    Enabled = false, -- État effectif (peut être modifié par la logique des HP)
...
local function UpdateTargetAndHighlight()
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                -- Vérifier les HP pour activer/désactiver ForceHit
                ...
                if health >= 0.5 and health <= 1.5 then
                    if ForceHitModule.Enabled then
                        ...
                        -- sendNotification("ForceHit", "Disabled (Target HP between 0.5 and 1.5)", 2)
                    end
                elseif health > 1.5 then
                    if not ForceHitModule.Enabled then
                        ...
                        -- sendNotification("ForceHit", "Enabled (Target HP above 1.5)", 2)
                    end
                end
            else
                ForceHitModule.SelectedTarget = nil
    ...
    -- Si ForceHit est désactivé (par la logique des HP ou autre), désactiver le Highlight
    if not ForceHitModule.Enabled then
        Highlight.Enabled = false
        return
    end
