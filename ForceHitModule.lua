...
local ForceHitModule = {
    ...
    TargetStrafeActive = false, -- Nouvelle variable pour suivre l'état actif du TargetStrafe
    BlankShots = true,
    HitPart = "Head",
    SelectedTarget = nil,
    TargetPlayer = nil,
    ...
    TargetInfoUI = nil,
    HitSound = nil,
    SavedPosition = nil, -- Pour stocker la position du joueur quand Target Strafe est activé
    LastAmmoValue = nil, -- Pour suivre la dernière valeur d'ammo connue
}
...
-- Valeurs d'ammo par défaut pour chaque arme
...
-- Fonction pour obtenir le nombre de munitions de l'arme équipée
local function GetEquippedWeaponAmmo(targetPlayer, equippedWeapon)
    ...
    -- Vérifier si l'arme est équipée dans les mains du personnage
    ...
    -- Si l'arme n'est pas équipée, vérifier dans le sac à dos (Backpack)
    ...
    -- Si aucune valeur n'est trouvée, utiliser la valeur par défaut
...
function ForceHitModule:UpdateTargetInfoUI()
    ...
    -- Mettre à jour l'affichage de l'arme et des munitions
...
-- Fonction pour surveiller en continu les munitions de la cible
local function MonitorTargetAmmo()
    ForceHitModule.Connections["MonitorAmmo"] = RunService.Heartbeat:Connect(function()
        ...
        -- Vérifier si la valeur des munitions a changé
        ...
        -- Mettre à jour l'étiquette de l'arme si l'arme équipée change
...
local function SetupUIMobileSupport()
    ...
    LocalPlayer.CharacterAdded:Connect(function()
        if ForceHitModule.UIMobileSupportEnabled and not ForceHitModule.ForceHitButton then
            print("Le joueur a respawn, recréation du bouton UI ForceHit")
            CreateForceHitButton()
        end
    end)
end
...
-- Section TargetStrafe modifiée pour gérer les HP et le forcefield de la cible
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    if not ForceHitModule.TargetStrafeEnabled or not ForceHitModule.TargetPlayer then 
        ForceHitModule.TargetStrafeActive = false
        return 
    end
    ...
    local targetForceField = targetCharacter and targetCharacter:FindFirstChildOfClass("ForceField")

    if not localRootPart or not targetRootPart or not targetHumanoid then
        ForceHitModule.TargetStrafeActive = false
        return
    end
    ...
    -- Vérifier si la cible a un forcefield actif
    if targetForceField then
        ForceHitModule.TargetStrafeActive = false
        return
    end

    -- Vérifier les HP de la cible
    local targetHealth = targetHumanoid.Health

    -- Si les HP sont à 0.5 ou moins, désactiver le TargetStrafe
    if targetHealth <= 0.5 then
        ForceHitModule.TargetStrafeActive = false
        return
    end

    -- Si les HP sont à 20 ou plus et qu'il n'y a pas de forcefield, activer le TargetStrafe
    if targetHealth >= 20 and not targetForceField then
        ForceHitModule.TargetStrafeActive = true
    end

    -- Si TargetStrafeActive est false (HP entre 0.5 et 20 ou forcefield actif), ne rien faire
    if not ForceHitModule.TargetStrafeActive then
        return
    end

    -- Si on arrive ici, TargetStrafeActive est true, donc on exécute le strafe
    local radius = 5
    local speed = 5
...
function ForceHitModule:Enable()
    ...
    -- Démarrer la surveillance des munitions lors de l'activation
    MonitorTargetAmmo()
end
...
function ForceHitModule:Disable()
    ...
    lastHealth = nil
    self.LastAmmoValue = nil -- Réinitialiser la dernière valeur d'ammo lors de la désactivation
end
...
function ForceHitModule:SetTargetStrafe(value)
    ...
    if value then
        -- Sauvegarder la position actuelle du joueur lors de l'activation du Target Strafe
        local localCharacter = LocalPlayer.Character
        local localRootPart = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart")
        ...
        self.StrafeAngle = 0
        self.TargetStrafeActive = true -- Activer par défaut
    else
        -- Téléporter le joueur à la position sauvegardée lors de la désactivation du Target Strafe
