...
local ForceHitModule = {
    ...
    HPRangeToDisable = {0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5} -- Plage de HP où les tirs sont désactivés
}
...
-- Fonction pour vérifier si les HP sont dans la plage désactivée
local function IsHPInDisabledRange(hp)
    for _, value in ipairs(ForceHitModule.HPRangeToDisable) do
        if math.abs(hp - value) < 0.01 then -- Utilisation d'une petite tolérance pour les comparaisons de nombres flottants
            return true
        end
    end
    return false
...
OriginalNameCall = hookmetamethod(game, "__namecall", function(Object, ...)
    ...
    if NameCallMethod == "FireServer" and Object.Name == "MainEvent" and #Arguments > 0 and Arguments[1] == "Shoot" then
        ...
        if AimPart then
            local character = AimPart.Parent
            local humanoid = character and character:FindFirstChild("Humanoid")
            if humanoid then
                local hp = humanoid.Health
                if IsHPInDisabledRange(hp) then
                    -- Ne pas rediriger les tirs si les HP sont dans la plage désactivée
                    return OriginalNameCall(Object, ...)
                end
            end
            -- Vérifier que Arguments[2] est un tableau et non nil
            if type(Arguments[2]) == "table" and #Arguments[2] > 0 then
                -- Vérifier que Arguments[2][1] et Arguments[2][2] sont des tableaux
                if type(Arguments[2][1]) == "table" and type(Arguments[2][2]) == "table" then
                    ...
                    print("Arguments[2][1] or Arguments[2][2] is not a table:", Arguments[2][1], Arguments[2][2])
                end
            else
                print("Arguments[2] is not a valid table or is empty:", Arguments[2])
            end
...
ForceHitModule.Connections["BlankShots"] = RunService.Heartbeat:Connect(function()
    ...
    if AimChar then
        ...
        local Humanoid = AimChar:FindFirstChild("Humanoid")
        if Humanoid then
            local hp = Humanoid.Health
            if IsHPInDisabledRange(hp) then
                -- Ne pas envoyer de tirs si les HP sont dans la plage désactivée
                return
            end
        end
