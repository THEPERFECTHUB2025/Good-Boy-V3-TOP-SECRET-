...
local ForceHitModule = {
    ...
    SelectedTarget = nil,
    TargetPlayer = nil,
    LastTargetPosition = nil, -- To store the last known position of the target for strafing after death
    Connections = {},
    ForceHitButton = nil,
    ...
    FrozenTargetRootPart = nil, -- To track the frozen target's HumanoidRootPart
}
...
-- Function to continuously monitor the ammo value of the equipped weapon
...
-- Function to freeze the target by anchoring their HumanoidRootPart
local function FreezeTarget()
    if not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then
        -- Unfreeze the target if ForceHit is disabled or no target is selected
        if ForceHitModule.FrozenTargetRootPart then
            ForceHitModule.FrozenTargetRootPart.Anchored = false
            ForceHitModule.FrozenTargetRootPart = nil
        end
        return
    end
    ...
    if not targetRootPart or not targetHumanoid or targetHumanoid.Health <= 0 then
        -- Unfreeze if the target is invalid or dead
        if ForceHitModule.FrozenTargetRootPart then
            ForceHitModule.FrozenTargetRootPart.Anchored = false
            ForceHitModule.FrozenTargetRootPart = nil
        end
    ...
    -- Freeze the target by anchoring their HumanoidRootPart
    if targetRootPart and not targetRootPart.Anchored then
        targetRootPart.Anchored = true
        ForceHitModule.FrozenTargetRootPart = targetRootPart
    end
end
...
local function UpdateTargetAndHighlight()
    if not ForceHitModule.ManuallyEnabled then
        ...
        FreezeTarget() -- Ensure the target is unfrozen when ForceHit is disabled
        return
    end
    ...
    if ForceHitModule.TargetPlayer then
        ...
        if character then
            ...
            local humanoid = character:FindFirstChild("Humanoid")
            local forceField = character:FindFirstChildOfClass("ForceField")
            if hitPart and humanoid and not forceField then
                ForceHitModule.SelectedTarget = hitPart
                -- Store the last known position of the target for strafing
                ForceHitModule.LastTargetPosition = hitPart.Position

                local health = humanoid.Health
                if health >= 0.5 and health <= 2.5 then
                ...
                if ForceHitModule.AutoTargetAll and health <= 0.5 then
                    ...
                    if newPlayer then
                        ...
                        ForceHitModule.LastTargetPosition = newPart and newPart.Position or nil
                    else
                ...
                if localPosition and hitPart then
                    ...
                    if IsTargetInVoid(targetPosition) then
                        ...
                        ForceHitModule.SelectedTarget = nil
                        ForceHitModule.TargetPlayer = nil
                        ForceHitModule.LastTargetPosition = nil
                        Highlight.Enabled = false
                        UpdateTracer()
                        ForceHitModule:UpdateTargetInfoUI()
                        FreezeTarget() -- Ensure the target is unfrozen if in void
                        return
                    end
                end
            else
    ...
    UpdateTracer()
    ForceHitModule:UpdateTargetInfoUI()
    FreezeTarget() -- Update the freeze state of the target
end
...
local function CheckTargetAfterRespawn()
    ...
    player.CharacterAdded:Connect(function(newCharacter)
        if not ForceHitModule.Enabled or not ForceHitModule.TargetPlayer then
            -- Unfreeze the old character if it exists
            if ForceHitModule.FrozenTargetRootPart then
                ForceHitModule.FrozenTargetRootPart.Anchored = false
                ForceHitModule.FrozenTargetRootPart = nil
            end
        ...
        -- Update the last known position when the target respawns
        local newHitPart = newCharacter:WaitForChild(ForceHitModule.HitPart, 5)
        if newHitPart then
            ForceHitModule.SelectedTarget = newHitPart
            ForceHitModule.LastTargetPosition = newHitPart.Position
            -- The new character will be frozen in UpdateTargetAndHighlight
        end
    end)
end
...
ForceHitModule.Connections["KillAll"] = RunService.Heartbeat:Connect(function()
    ...
    if localRootPart and targetRootPart then
        ...
        ForceHitModule.LastTargetPosition = closestPart and closestPart.Position or nil
...
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    -- Use the last known position if the target is dead or not available
    local strafePosition
    if targetRootPart then
        strafePosition = targetRootPart.Position
        ForceHitModule.LastTargetPosition = strafePosition
    elseif ForceHitModule.LastTargetPosition then
        strafePosition = ForceHitModule.LastTargetPosition
    else
        return -- No position to strafe around
    end

    local distance = (localRootPart.Position - strafePosition).Magnitude
    if distance > 100000 then -- Maximum distance of 100,000 studs
        return
    end

    if IsTargetInVoid(strafePosition) then
        return
    end
    ...
    local newPosition = strafePosition + offset

    localRootPart.CFrame = CFrame.new(newPosition, strafePosition)
    localRootPart.CFrame = localRootPart.CFrame + Vector3.new(0, strafePosition.Y - localRootPart.Position.Y, 0)
end)
...
function ForceHitModule:Enable()
    ...
    if not self.TargetPlayer then
        ...
        self.LastTargetPosition = ClosestPart and ClosestPart.Position or nil
        CheckTargetAfterRespawn()
    end

    -- Start monitoring ammo when enabled
    MonitorTargetAmmo()
end
...
function ForceHitModule:Disable()
    ...
    -- Unfreeze the target when disabling
    if self.FrozenTargetRootPart then
        self.FrozenTargetRootPart.Anchored = false
        self.FrozenTargetRootPart = nil
    end
end
...
function ForceHitModule:SetTargetStrafe(value)
    ...
    if value then
        ...
        self.LastTargetPosition = nil -- Reset the last target position when disabling
    end
end
...
function ForceHitModule:Cleanup()
    ...
    -- Ensure the target is unfrozen during cleanup
    if self.FrozenTargetRootPart then
        self.FrozenTargetRootPart.Anchored = false
        self.FrozenTargetRootPart = nil
    end
