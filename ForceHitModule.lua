...
local ForceHitModule = {
    Enabled = false, -- État effectif (peut être modifié par la logique des HP ou de la distance)
    ManuallyEnabled = false, -- État défini par l'utilisateur via le keybind
    AutoTargetAll = false, -- Nouvelle option pour cibler automatiquement une nouvelle personne
    BlankShots = true,
    HitPart = "Head",
...
local function UpdateTargetAndHighlight()
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if ForceHitModule.AutoTargetAll and health <= 0.5 then
                    ...
                    if newPlayer then
                        ...
                        -- Optionnel : Ajouter une notification pour indiquer le changement de cible
                        -- sendNotification("ForceHit", "Target switched to " .. newPlayer.Name .. " (previous target HP <= 0.5)", 2)
                    else
                ...
                if localPosition and hitPart then
                    ...
                    if distance > 200 then
                        if ForceHitModule.Enabled then
                            ...
                            -- sendNotification("ForceHit", "Disabled (Target too far: " .. math.floor(distance) .. "m)", 2)
                        end
                    elseif distance <= 200 then
                        if not ForceHitModule.Enabled and health > 1.5 then -- Ne réactiver que si les HP sont aussi > 1.5
                            ForceHitModule.Enabled = true
                            -- Optionnel : Ajouter une notification pour indiquer que ForceHit est réactivé
                            -- sendNotification("ForceHit", "Enabled (Target within 200m: " .. math.floor(distance) .. "m)", 2)
                        end
                    end
                end
            else
    ...
    -- Si ForceHit est désactivé (par la logique des HP, de la distance ou autre), désactiver le Highlight
    if not ForceHitModule.Enabled then
        Highlight.Enabled = false
        return
    end
