...
-- Create the draggable UI button
...
-- Create the target info UI (modified to position below HP bar and add weapon info)
local function CreateTargetInfoUI()
    ...
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(0, 300, 0, 180) -- Increased height to accommodate weapon info
    -- Position below the HP bar
    local hpFrame = game:GetService("Players").LocalPlayer.PlayerGui["Main Screen"].Bar.HP
    frame.Position = UDim2.new(0, hpFrame.Position.X.Offset, 0, hpFrame.Position.Y.Offset + hpFrame.Size.Y.Offset + 10)
    ...
    local function shakeEffect()
        while true do
            ...
            local tween = TweenService:Create(frame, tweenInfo, {Position = UDim2.new(0, frame.Position.X.Offset + offsetX, 0, frame.Position.Y.Offset + offsetY)})
            tween:Play()
    ...
    weaponLabel.Position = UDim2.new(0, 40, 0, 110) -- Positioned below health and studs
    weaponLabel.BackgroundTransparency = 1
    weaponLabel.Text = "nil"
    weaponLabel.TextColor3 = Color3.fromRGB(255, 215, 0) -- Gold color for weapon
    ...
    local function glitchWeaponEffect()
        while true do
            ...
            weaponLabel.Position = UDim2.new(0, 40 + math.random(-1, 1), 0, 110)
            wait(0.05)
            weaponLabel.TextTransparency = 0
            weaponLabel.Position = UDim2.new(0, 40, 0, 110)
            wait(0.05)
    ...
    ForceHitModule.TargetInfoUI = {
        ...
        WeaponLabel = weaponLabel -- Add weapon label to the UI table
    }

    frame.Visible = false
end

-- Update the target info UI with weapon information
...
ForceHitModule.Connections["HitSound"] = RunService.Heartbeat:Connect(function()
    ...
    if currentHealth < lastHealth and currentHealth > 0 then
        ...
        print("ForceHit: Hit détecté sur " .. ForceHitModule.TargetPlayer.Name .. ", santé réduite de " .. lastHealth - currentHealth)
    end
