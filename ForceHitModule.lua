...
-- Fonction pour créer des particules d'énergie (déplacée hors de CreateTargetInfoUI)
local function createEnergyParticle(energyParticleFrame)
    ...
    particle.Parent = energyParticleFrame
...
-- Créer l'UI pour les informations sur la cible (design futuriste perfectionné)
local function CreateTargetInfoUI()
    ...
    gradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 50, 200)),
    ...
    -- Ajouter des particules d'énergie (quand la cible est proche)
    local energyParticleFrame = Instance.new("Frame")
    energyParticleFrame.Size = UDim2.new(1, 0, 1, 0)
    energyParticleFrame.BackgroundTransparency = 1
    energyParticleFrame.Parent = frame
    ...
    -- Ajouter un effet de "synchronisation" au démarrage
    local syncFrame = Instance.new("Frame")
    syncFrame.Size = UDim2.new(1, 0, 1, 0)
    syncFrame.BackgroundTransparency = 1
    syncFrame.Parent = frame

    local function createSyncCircle()
        local circle = Instance.new("Frame")
        circle.Size = UDim2.new(0, 0, 0, 0)
        circle.Position = UDim2.new(0.5, 0, 0.5, 0) -- Centré sur l'UI
        circle.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
        circle.BackgroundTransparency = 0.3
        circle.Parent = syncFrame
    ...
    for i = 1, 4 do
        ...
        segment.Position = UDim2.new(0.5, math.cos(angle) * 25, 0.5, math.sin(angle) * 25)
        segment.Rotation = angle * (180 / math.pi)

        local function rotateSegment()
            while true do
                local tweenInfo = TweenInfo.new(2, Enum.EasingStyle.Linear)
                local tween = TweenService:Create(segment, tweenInfo, {Rotation = segment.Rotation + 360})
                tween:Play()
                tween.Completed:Wait()
            end
        end
        ...
        local function flickerSegment()
            while true do
                local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
                local tween1 = TweenService:Create(segment, tweenInfo, {BackgroundTransparency = 0.5})
                local tween2 = TweenService:Create(segment, tweenInfo, {BackgroundTransparency = 0.2})
                tween1:Play()
                tween1.Completed:Wait()
                tween2:Play()
                tween2.Completed:Wait()
    ...
    ForceHitModule.TargetInfoUI = {
        ...
        EnergyParticleFrame = energyParticleFrame
    }
...
local function UpdateTargetInfoUI()
    ...
    -- Ajouter des particules d'énergie si la cible est proche (≤ 10 studs)
    if distance <= 10 then
        for i = 1, 2 do
            createEnergyParticle(ui.EnergyParticleFrame) -- Appeler directement la fonction
        end
    end
    ...
    if not ui.Frame.Visible then
        ...
        tween:Play()
        -- Lancer l'effet de synchronisation
        for i = 1, 3 do
            local createSyncCircle = ui.Frame.Parent.Parent.Parent.Parent.CreateTargetInfoUI.createSyncCircle
            createSyncCircle()
            wait(0.2)
        end
    end
