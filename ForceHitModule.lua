...
local function CreateTargetInfoUI()
    ...
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(0, 300, 0, 220) -- Increased height to accommodate new labels
    ...
    local function adjustPositionForScreen()
        ...
        if viewportSize.X < 600 then
            ...
            frame.Position = UDim2.new(0, 5, 0.5, -95)
        else
    ...
    local function shakeEffect()
        while true do
            ...
            local tween = TweenService:Create(frame, tweenInfo, {Position = UDim2.new(0, 10 + offsetX, 0.5, -110 + offsetY)})
            tween:Play()
    ...
    -- New Ammo Label for displaying ammo counts
    local ammoLabel = Instance.new("TextLabel")
    ammoLabel.Size = UDim2.new(0, 220, 0, 75) -- Larger height to accommodate multiple lines
    ammoLabel.Position = UDim2.new(0, 70, 0, 125)
    ammoLabel.BackgroundTransparency = 1
    ammoLabel.Text = "Ammo: N/A"
    ...
    ammoLabel.TextYAlignment = Enum.TextYAlignment.Top
    ammoLabel.Font = Enum.Font.Code
    ammoLabel.Parent = frame

    local function glitchAmmoEffect()
        while true do
            ammoLabel.TextTransparency = 0.2
            ammoLabel.Position = UDim2.new(0, 70 + math.random(-1, 1), 0, 125)
            wait(0.05)
            ammoLabel.TextTransparency = 0
            ammoLabel.Position = UDim2.new(0, 70, 0, 125)
            wait(0.05)
            ammoLabel.TextTransparency = 0.3
            ammoLabel.Position = UDim2.new(0, 70 + math.random(-1, 1), 0, 125)
            wait(0.05)
            ammoLabel.TextTransparency = 0
            ammoLabel.Position = UDim2.new(0, 70, 0, 125)
    ...
    ForceHitModule.TargetInfoUI = {
        ...
        AmmoLabel = ammoLabel -- Add the new ammo label to the UI table
    }
...
-- Function to get the ammo count for each weapon in the target's backpack
local function GetTargetAmmo(targetPlayer)
    if not targetPlayer then return "N/A" end
    ...
    for _, weaponName in pairs(WeaponsList) do
        ...
        if weapon then
            ...
            if ammo and ammo:IsA("IntValue") then
                ammoInfo = ammoInfo .. weaponName .. ": " .. ammo.Value .. "\n"
            else
                ammoInfo = ammoInfo .. weaponName .. ": N/A\n"
            end
        else
            ammoInfo = ammoInfo .. weaponName .. ": N/A\n"
        end
...
function ForceHitModule:UpdateTargetInfoUI()
    ...
    if not getgenv().Rake.Settings.Misc.TargetInfoEnabled or not self.ManuallyEnabled or not self.TargetPlayer then
        if ui.Frame.Visible then
            ...
            tween.Completed:Connect(function()
                ...
                ui.Frame.Size = Camera.ViewportSize.X < 600 and UDim2.new(0, 250, 0, 190) or UDim2.new(0, 300, 0, 220)
    ...
    -- Update the ammo label with the target's ammo counts
    ui.AmmoLabel.Text = "Ammo:\n" .. GetTargetAmmo(targetPlayer)
    ...
    if health <= maxHealth * 0.25 and ui.Frame.Visible then
        ...
        local tween1 = TweenService:Create(ui.Frame, tweenInfo, {Size = Camera.ViewportSize.X < 600 and UDim2.new(0, 260, 0, 195) or UDim2.new(0, 310, 0, 225)})
        local tween2 = TweenService:Create(ui.Frame, tweenInfo, {Size = Camera.ViewportSize.X < 600 and UDim2.new(0, 250, 0, 190) or UDim2.new(0, 300, 0, 220)})
        tween1:Play()
    ...
    if not ui.Frame.Visible then
        ...
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 0.5, Size = Camera.ViewportSize.X < 600 and UDim2.new(0, 250, 0, 190) or UDim2.new(0, 300, 0, 220)})
        tween:Play()
        tween.Completed:Connect(function()
            particleFrame:Destroy()
        end)
