...
local function UpdateTargetAndHighlight()
    ...
    if ForceHitModule.TargetPlayer then
        -- Vérifier si la cible est whitelistée (si tu as implémenté la whitelist dans la première partie)
        if ForceHitModule.Whitelist and IsPlayerWhitelisted(ForceHitModule.TargetPlayer) then
            ForceHitModule.TargetPlayer = nil
            ForceHitModule.SelectedTarget = nil
            -- Trouver une nouvelle cible non whitelistée
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if localPosition and hitPart then
                    ...
                    -- On ne désactive plus ForceHitModule.Enabled si la distance dépasse 200 mètres
                    -- La logique de tir sera gérée dans BlankShots
...
ForceHitModule.Connections["BlankShots"] = RunService.Heartbeat:Connect(function()
    ...
    if AimChar then
        local targetPlayer = Players:GetPlayerFromCharacter(AimChar)
        if ForceHitModule.Whitelist and targetPlayer and IsPlayerWhitelisted(targetPlayer) then return end

        local localCharacter = LocalPlayer.Character
        local localRootPart = localCharacter and localCharacter:FindFirstChild("HumanoidRootPart")
        if not localRootPart or not AimPart then return end

        -- Vérifier la distance avant de tirer
        local distance = (localRootPart.Position - AimPart.Position).Magnitude
        if distance > 714.2 then return end -- 200 mètres ≈ 714.2 studs
