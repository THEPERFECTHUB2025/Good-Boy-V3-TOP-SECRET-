...
-- Modified KillAll to target players within 500 meters (1785.5 studs) with at least 20 HP
ForceHitModule.Connections["KillAll"] = RunService.Heartbeat:Connect(function()
    ...
    local maxDistance = 1785.5 -- 500 meters in studs (1 meter = 3.571 studs)
    ...
    for _, player in pairs(Players:GetPlayers()) do
        ...
        if not (humanoid and targetRootPart and hitPart and not forceField) then continue end
        if humanoid.Health < 20 then continue end -- Skip players with less than 20 HP

        local distance = (localPosition - targetRootPart.Position).Magnitude
        if distance > maxDistance then continue end -- Skip players beyond 500 meters

        if IsTargetInVoid(targetRootPart.Position) then continue end

        localRootPart.CFrame = targetRootPart.CFrame * CFrame.new(0, 0, -2)
        ForceHitModule.TargetPlayer = player
        ForceHitModule.SelectedTarget = hitPart
...
-- Modified TargetStrafe to only activate if the target has at least 20 HP
ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if not localRootPart or not targetRootPart or not targetHumanoid then return end

    -- Check if the target has at least 20 HP
    if targetHumanoid.Health < 20 then return end
