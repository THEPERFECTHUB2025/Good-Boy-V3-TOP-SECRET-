...
local function UpdateTargetAndHighlight()
    if not ForceHitModule.ManuallyEnabled then
        ...
        UpdateTracer()
        if ForceHitModule.UpdateTargetInfoUI then
            ForceHitModule:UpdateTargetInfoUI()
        else
            warn("[UpdateTargetAndHighlight] UpdateTargetInfoUI is not defined!")
        end
        return
    end
        ...
        if character then
            ...
            if hitPart and humanoid and humanoid.Health > 0 and not forceField then
                ...
                if localPosition and hitPart then
                    ...
                    if ForceHitModule.WallCheckEnabled then
                        if not localPosition or not targetPosition then
                            warn("[UpdateTargetAndHighlight] Missing localPosition or targetPosition")
                        elseif not IsTargetBehindWall then
                            warn("[UpdateTargetAndHighlight] IsTargetBehindWall is nil!")
                        else
...
-- Move this function to just after UpdateTargetAndHighlight
...
-- Update BlankShots to check for Silent Aim
ForceHitModule.Connections["BlankShots"] = RunService.Heartbeat:Connect(function()
    ...
    -- Skip if Silent Aim is enabled (adjust the condition based on your Silent Aim implementation)
    if getgenv().Rake.Settings.Misc.SilentAimEnabled then return end
...
function ForceHitModule:Cleanup()
    self:Disable() -- Désactive le module
    ...
    self.Connections = {} -- Réinitialise la table des connexions
    ...
    self.OriginalEnabledState = nil -- Réinitialise l'état sauvegardé
    self.Whitelist = {} -- Réinitialise la liste blanche
...
local function CreateTargetInfoUI()
    ...
    -- Variables pour contrôler les animations
    local isAnimating = true
    local activeParticles = 0
    local maxParticles = 10 -- Limite de particules simultanées
    ...
    -- Rotation du gradient (réduite à une fois toutes les 8 secondes)
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
            ...
            wait(2) -- Pause entre les rotations
        end
    end)()

    -- Rotation du stroke gradient (réduite à une fois toutes les 4 secondes)
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
    ...
    -- Flicker du bord (réduit à toutes les 0.8 secondes)
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
    ...
    -- Effet hologram (réduit à toutes les 2 secondes)
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
    ...
    local function createParticle()
        if activeParticles >= maxParticles then return end
    ...
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
            createParticle()
            wait(0.3) -- Réduit la fréquence des particules
        end
    end)()
    ...
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
            createWave()
            wait(1.5) -- Réduit la fréquence des vagues
        end
    end)()
    ...
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
            createRadialScan()
            wait(3) -- Réduit la fréquence des scans
        end
    end)()
    ...
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
            createGlitchLine()
            wait(math.random(2, 5)) -- Réduit la fréquence des glitchs
        end
    end)()

    -- Réduire la fréquence du shake (toutes les 5 secondes)
    coroutine.wrap(function()
        while true do
            if not isAnimating or not frame.Parent then break end
    ...
    frame:GetPropertyChangedSignal("Visible"):Connect(function()
        if frame.Visible then
            ForceHitModule.TargetInfoUI.StartAnimations()
        else
            ForceHitModule.TargetInfoUI.StopAnimations()
        end
    end)
...
function ForceHitModule:UpdateTargetInfoUI()
    ...
    if not ui.Frame.Visible then
        ...
        local tween = TweenService:Create(ui.Frame, tweenInfo, {
            ...
            Size = Camera.ViewportSize.X < 600 and UDim2.new(0, 250, 0, 150) or UDim2.new(0, 300, 0, 180)
        })
        tween:Play()
        tween.Completed:Connect(function()
            particleFrame:Destroy()
...
-- Move this block to just after GetClosestPlayerByDistance
...
local function StartUpdateTargetAndHighlightConnection()
    if ForceHitModule.Connections["UpdateTargetAndHighlight"] then
        ForceHitModule.Connections["UpdateTargetAndHighlight"]:Disconnect()
    end
    local lastUpdateTime = 0
    ForceHitModule.Connections["UpdateTargetAndHighlight"] = RunService.Heartbeat:Connect(function()
        local currentTime = os.clock()
        if currentTime - lastUpdateTime < 0.1 then return end -- Update every 0.1 seconds
...
-- Remove the print statement and optimize the TargetStrafe connection
ForceHitModule.Connections["TargetStrafe"] = nil -- Initially set to nil

local function StartTargetStrafeConnection()
    if ForceHitModule.Connections["TargetStrafe"] then
        ForceHitModule.Connections["TargetStrafe"]:Disconnect()
    end
    ForceHitModule.Connections["TargetStrafe"] = RunService.Heartbeat:Connect(function(deltaTime)
        if not ForceHitModule.TargetStrafeEnabled or not ForceHitModule.Enabled or not ForceHitModule.TargetPlayer then
            return
...
-- Modify SetTargetStrafe to manage the connection
function ForceHitModule:SetTargetStrafe(value)
    ...
    if value then
        ...
        self.StrafeAngle = 0
        StartTargetStrafeConnection() -- Start the connection when enabling
    else
        ...
        -- Disconnect the connection when disabling
        if self.Connections["TargetStrafe"] then
            self.Connections["TargetStrafe"]:Disconnect()
            self.Connections["TargetStrafe"] = nil
        end
    end
end
...
function ForceHitModule:Enable()
    ...
    if not self.TargetPlayer then
        ...
        if self.TargetPlayer and MonitorTargetRespawn then
            MonitorTargetRespawn()
        else
            warn("[ForceHitModule:Enable] No target selected or MonitorTargetRespawn is nil")
        end
    end
    MonitorTargetAmmo()
    StartUpdateTargetAndHighlightConnection()
...
function ForceHitModule:Disable()
    ...
    lastHealth = nil
    self.LastAmmoValue = nil
    -- Disconnect the UpdateTargetAndHighlight connection when disabling
    if self.Connections["UpdateTargetAndHighlight"] then
        self.Connections["UpdateTargetAndHighlight"]:Disconnect()
        self.Connections["UpdateTargetAndHighlight"] = nil
    end
end
...
function ForceHitModule:Cleanup()
    ...
    self.Connections = {} -- Ajouté pour réinitialiser la table
    if self.Highlight then
        self.Highlight:Destroy()
        self.Highlight = nil
    end
    ...
    self.SavedPosition = nil
    self.OriginalEnabledState = nil -- Ajouté
    self.Whitelist = {} -- Ajouté
    self.TargetPlayer = nil -- Ajouté
    self.SelectedTarget = nil -- Ajouté
    self.Enabled = false -- Ajouté
    self.ManuallyEnabled = false -- Ajouté
end
