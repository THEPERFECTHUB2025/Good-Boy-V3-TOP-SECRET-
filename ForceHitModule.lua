...
local function CreateTargetInfoUI()
    ...
    -- Ajouter un gradient de fond vibrant avec des couleurs néon
    local gradient = Instance.new("UIGradient")
    gradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 255, 255)), -- Cyan
        ColorSequenceKeypoint.new(0.33, Color3.fromRGB(255, 0, 255)), -- Magenta
        ColorSequenceKeypoint.new(0.66, Color3.fromRGB(0, 255, 100)), -- Vert acide
        ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 255, 255)) -- Retour au cyan
    })
    ...
    -- Animation de pulsation pour la bordure (variation de l'intensité des couleurs)
    local function pulseBorder()
        while true do
            local tweenInfo = TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
            local tween1 = TweenService:Create(stroke, tweenInfo, {Transparency = 0.1})
    ...
    -- Ajouter un effet de distorsion (flottement subtil)
    local function distortionEffect()
        while true do
            local tweenInfo = TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
            local offsetX = math.random(-3, 3)
            local offsetY = math.random(-3, 3)
            local scale = math.random(98, 102) / 100 -- Variation de taille entre 98% et 102%
            local tween1 = TweenService:Create(frame, tweenInfo, {
                Position = UDim2.new(0, 10 + offsetX, 0, 10 + offsetY),
                Size = UDim2.new(0, 320 * scale, 0, 160 * scale)
            })
            tween1:Play()
            tween1.Completed:Wait()
        end
    end
    spawn(distortionEffect)
    ...
    local function createConnectionLine()
        ...
        local endX = 35 + math.cos(angle) * 160
        local endY = 75 + math.sin(angle) * 160
    ...
    -- Ajouter un effet de "réseau neuronal" (lignes interconnectées)
    local neuralFrame = Instance.new("Frame")
    neuralFrame.Size = UDim2.new(1, 0, 1, 0)
    neuralFrame.BackgroundTransparency = 1
    neuralFrame.Parent = frame

    local function createNeuralLine()
        local startX = math.random() * 320
        local startY = math.random() * 160
        local endX = math.random() * 320
        local endY = math.random() * 160

        local line = Instance.new("Frame")
        line.Size = UDim2.new(0, 1, 0, math.sqrt((endX - startX)^2 + (endY - startY)^2))
        line.Position = UDim2.new(0, startX, 0, startY)
        line.Rotation = math.deg(math.atan2(endY - startY, endX - startX))
        line.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
        line.BackgroundTransparency = 0.7
        line.Parent = neuralFrame

        local tweenInfo = TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TweenService:Create(line, tweenInfo, {BackgroundTransparency = 1})
        tween:Play()
        tween.Completed:Connect(function()
            line:Destroy()
        end)
    ...
    local function spawnNeuralLines()
        while true do
            createNeuralLine()
            wait(0.3)
        end
    end
    spawn(spawnNeuralLines)

    -- Ajouter un effet de "chaleur" (lignes ondulantes) lorsque la cible est proche
    local heatFrame = Instance.new("Frame")
    heatFrame.Size = UDim2.new(1, 0, 1, 0)
    heatFrame.BackgroundTransparency = 1
    heatFrame.Parent = frame

    local function createHeatWave()
        local heatWave = Instance.new("Frame")
        heatWave.Size = UDim2.new(1, 0, 0, 2)
        heatWave.Position = UDim2.new(0, 0, math.random(), 0)
        heatWave.BackgroundColor3 = Color3.fromRGB(255, 0, 255)
        heatWave.BackgroundTransparency = 0.8
        heatWave.Parent = heatFrame

        local tweenInfo = TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
        local offsetX = math.random(-10, 10)
        local tween = TweenService:Create(heatWave, tweenInfo, {
            Position = UDim2.new(0, offsetX, heatWave.Position.Y.Scale, 0),
            BackgroundTransparency = 1
        })
        tween:Play()
        tween.Completed:Connect(function()
    ...
    local function spawnHeatWaves()
        while true do
            if ForceHitModule.TargetInfoUI and ForceHitModule.TargetInfoUI.DistanceLabel then
                local distanceText = ForceHitModule.TargetInfoUI.DistanceLabel.Text
                local distance = tonumber(distanceText:match("%d+"))
    ...
    -- Ajouter un réticule rotatif autour de l'icône
    local reticle = Instance.new("Frame")
    reticle.Size = UDim2.new(0, 60, 0, 60)
    reticle.Position = UDim2.new(0, 5, 0, 45)
    reticle.BackgroundTransparency = 1
    reticle.Parent = frame

    local reticleStroke = Instance.new("UIStroke")
    reticleStroke.Thickness = 2
    reticleStroke.Color = Color3.fromRGB(0, 255, 255)
    reticleStroke.Transparency = 0.3
    ...
    local function rotateReticle()
        while true do
            local tweenInfo = TweenInfo.new(2, Enum.EasingStyle.Linear)
            local tween = TweenService:Create(reticle, tweenInfo, {Rotation = reticle.Rotation + 360})
            tween:Play()
            tween.Completed:Wait()
        end
    end
    ...
    nameLabel.Size = UDim2.new(0, 240, 0, 25)
    ...
    nameLabel.TextTransparency = 1 -- Caché par défaut pour l'animation
    ...
    healthLabel.Size = UDim2.new(0, 120, 0, 25)
    healthLabel.Position = UDim2.new(0, 70, 0, 80)
    ...
    healthLabel.TextTransparency = 1 -- Caché par défaut pour l'animation
    ...
    local function glitchHealthEffect()
        while true do
            ...
            healthLabel.Position = UDim2.new(0, 70 + math.random(-1, 1), 0, 80)
            wait(0.05)
    ...
    distanceLabel.Size = UDim2.new(0, 110, 0, 25)
    distanceLabel.Position = UDim2.new(0, 200, 0, 80)
    ...
    distanceLabel.TextTransparency = 1 -- Caché par défaut pour l'animation
    ...
    local function glitchDistanceEffect()
        while true do
            ...
            distanceLabel.Position = UDim2.new(0, 200 + math.random(-1, 1), 0, 80)
            wait(0.05)
...
local function UpdateTargetInfoUI()
    ...
    if not ForceHitModule.ManuallyEnabled or not ForceHitModule.TargetPlayer then
        if ui.Frame.Visible then
            ...
            local tween = TweenService:Create(ui.Frame, tweenInfo, {BackgroundTransparency = 1, Position = UDim2.new(0, -320, 0, 10)})
            tween:Play()
            tween.Completed:Connect(function()
                ui.Frame.Visible = false
                ui.Frame.BackgroundTransparency = 0.5
                ui.Frame.Position = UDim2.new(0, 10, 0, 10)
                ui.NameLabel.TextTransparency = 1
                ui.HealthLabel.TextTransparency = 1
                ui.DistanceLabel.TextTransparency = 1
    ...
    -- Mettre à jour le nom avec une animation de fade-in
    if ui.NameLabel.Text ~= targetPlayer.Name .. " (@" .. targetPlayer.DisplayName .. ")" then
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
        local tweenOut = TweenService:Create(ui.NameLabel, tweenInfo, {TextTransparency = 1})
        tweenOut:Play()
        tweenOut.Completed:Connect(function()
            ui.NameLabel.Text = targetPlayer.Name .. " (@" .. targetPlayer.DisplayName .. ")"
            local tweenIn = TweenService:Create(ui.NameLabel, tweenInfo, {TextTransparency = 0})
            tweenIn:Play()
        end)
    end

    -- Mettre à jour la santé avec une animation de fade-in
    local health = math.floor(humanoid.Health)
    local maxHealth = math.floor(humanoid.MaxHealth)
    if ui.HealthLabel.Text ~= health .. "/" .. maxHealth then
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
        local tweenOut = TweenService:Create(ui.HealthLabel, tweenInfo, {TextTransparency = 1})
        tweenOut:Play()
        tweenOut.Completed:Connect(function()
            ui.HealthLabel.Text = health .. "/" .. maxHealth
            local tweenIn = TweenService:Create(ui.HealthLabel, tweenInfo, {TextTransparency = 0})
            tweenIn:Play()
        end)
    end
    ...
    -- Mettre à jour la distance avec une animation de fade-in
    local distance = (localRootPart.Position - targetRootPart.Position).Magnitude
    local distanceText = math.floor(distance) .. " studs"
    if ui.DistanceLabel.Text ~= distanceText then
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
        local tweenOut = TweenService:Create(ui.DistanceLabel, tweenInfo, {TextTransparency = 1})
        tweenOut:Play()
        tweenOut.Completed:Connect(function()
            ui.DistanceLabel.Text = distanceText
            local tweenIn = TweenService:Create(ui.DistanceLabel, tweenInfo, {TextTransparency = 0})
            tweenIn:Play()
        end)
    end
    ...
    if not ui.Frame.Visible then
        ...
        ui.Frame.Position = UDim2.new(0, -320, 0, 10)
        local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        ...
        TweenService:Create(ui.NameLabel, fadeInInfo, {TextTransparency = 0}):Play()
        TweenService:Create(ui.HealthLabel, fadeInInfo, {TextTransparency = 0}):Play()
        TweenService:Create(ui.DistanceLabel, fadeInInfo, {TextTransparency = 0}):Play()
    end
end
