...
local ToggleKey = Enum.KeyCode.G -- Touche par défaut (changée en G)
...
local function runCSync()
    ...
    CameraPart.Position = originalCFrame.PositionเลยVector3.new(0, 2, 0)
...
local function setKeybindEnabled(enabled)
    ...
    print("KeybindEnabled défini sur: " .. tostring(KeybindEnabled)) -- Débogage
    if enabled then
...
local function setKeybind(keyCode)
    ...
    if typeof(keyCode) == "EnumItem" and keyCode.EnumType == Enum.KeyCode then
        ...
        print("Touche de toggle mise à jour: " .. keyCode.Name) -- Débogage
    else
...
local function setupKeyBindings()
    local keyConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if gameProcessed then return end -- Ignore si l'entrée est utilisée par le jeu (par exemple, chat)
        print("KeybindEnabled: " .. tostring(KeybindEnabled)) -- Débogage
        print("Touche pressée: " .. tostring(input.KeyCode)) -- Débogage
        if not KeybindEnabled then return end
        
        if input.KeyCode == ToggleKey then
            print("Touche de toggle détectée: " .. input.KeyCode.Name) -- Débogage
            toggleCSync()
