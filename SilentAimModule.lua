...
local tracerUpdateConnection = nil -- Pour maintenir le tracer actif
        ...
        if Player ~= LocalPlayer then
            ...
            if Character and Character:FindFirstChild("Humanoid") and Character.Humanoid.Health > 0 then
                ...
                if Part then
                    ...
                    -- Supprimer la vérification OnScreen pour permettre une distance infinie
...
-- Fonction pour maintenir le tracer actif
local function startTracerUpdateLoop()
    ...
    tracerUpdateConnection = RunService.Heartbeat:Connect(function()
        ...
        if not targetCharacter or not targetHumanoid or targetHumanoid.Health <= 0 then
            print("Silent Aim: Cible", SilentAimLockedTarget.Name, "morte ou invalide - En attente de respawn")
            return
        end

        -- Vérifier si le tracer existe, sinon le recréer
        local hasTracer = false
        for _, obj in pairs(targetCharacter:GetChildren()) do
        ...
        if not hasTracer then
            print("Silent Aim: Tracer manquant pour", SilentAimLockedTarget.Name, "- Recréation")
...
function SilentAimModule:Enable()
    ...
    localPlayerCharacterConnection = LocalPlayer.CharacterAdded:Connect(function(newCharacter)
        print("Silent Aim: LocalPlayer a respawné")
        task.wait(0.1) -- Attendre que le personnage soit complètement chargé
        if SilentAimEnabled and SilentAimLockedTarget then
            print("Silent Aim: Rétablissement du tracer après respawn du LocalPlayer")
            highlightSilentAimTarget(SilentAimLockedTarget)
            createTracerSilentAimTarget(SilentAimLockedTarget)
        end
    end)

    -- Relancer la boucle de mise à jour du tracer si une cible est déjà sélectionnée
...
function SilentAimModule:Disable()
    ...
    -- Arrêter la boucle de mise à jour du tracer
    if tracerUpdateConnection then
        tracerUpdateConnection:Disconnect()
        tracerUpdateConnection = nil
    end
...
function SilentAimModule:ToggleTarget()
    ...
    if SilentAimTarget then
        ...
        if SilentAimTarget then
            ...
            -- Démarrer la boucle de mise à jour du tracer
            startTracerUpdateLoop()
        else
