-- Dont fucking use my silent aim fucking skid cause i cant obfuscate it and its cant be ofbuscated due to -60 fps drop 
...
function SilentAimModule:ToggleTarget()
    ...
    if SilentAimModule.SilentAimTarget then
        ...
        -- Do not clean up indicators if Silent Aim is still enabled
        if not SilentAimEnabled and SilentAimModule.SilentAimTarget and SilentAimModule.SilentAimTarget.Character then
            cleanHighlightsAndTracers(SilentAimModule.SilentAimTarget)
        end
