...
-- Reference to ForceHitModule (assuming it's available in the environment)
local ForceHitModule = getgenv().ForceHitModule -- Adjust this based on how ForceHitModule is loaded
...
local lastHookTime = 0 -- Track last hook call to limit frequency
...
local function createTracerSilentAimTarget(plr)
    ...
    -- Clean up any existing tracers before creating a new one
    cleanHighlightsAndTracers(plr)
...
function SilentAimModule:Enable()
    ...
    originalIndex = hookmetamethod(game, "__index", function(self, key)
        -- Safeguard: limit hook call frequency to avoid stack overflows
    ...
    -- Add indicators immediately if a target is selected
    if SilentAimModule.SilentAimTarget then
        print("Silent Aim: Enabling indicators for target " .. SilentAimModule.SilentAimTarget.Name .. " upon Silent Aim activation")
        highlightSilentAimTarget(SilentAimModule.SilentAimTarget)
        createTracerSilentAimTarget(SilentAimModule.SilentAimTarget)

        -- Enable ForceHitModule indicators on the target
        if ForceHitModule then
            ForceHitModule.TargetPlayer = SilentAimModule.SilentAimTarget
            ForceHitModule.SelectedTarget = SilentAimModule.SilentAimTarget.Character:FindFirstChild(getgenv().Rake.Settings.AimPart or "Head")
            ForceHitModule.Enabled = true
            ForceHitModule.ManuallyEnabled = true
            print("Silent Aim: ForceHitModule indicators enabled for target " .. SilentAimModule.SilentAimTarget.Name)
        else
            warn("Silent Aim: ForceHitModule not found - Cannot enable indicators")
        end
    end
    ...
    localPlayerCharacterAddedConnection = LocalPlayer.CharacterAdded:Connect(function(newCharacter)
        ...
        if SilentAimEnabled and SilentAimModule.SilentAimTarget then
            print("Silent Aim: Recreating tracer for target " .. SilentAimModule.SilentAimTarget.Name .. " after LocalPlayer respawn")
            highlightSilentAimTarget(SilentAimModule.SilentAimTarget)
            createTracerSilentAimTarget(SilentAimModule.SilentAimTarget)
        end
    end)
...
function SilentAimModule:Disable()
    ...
    -- Clean up all CharacterAdded connections
    ...
    -- Disable ForceHitModule indicators
    if ForceHitModule then
        ...
        print("Silent Aim: ForceHitModule indicators disabled")
    end
    ...
    if SilentAimModule.SilentAimTarget then
        ...
        if SilentAimModule.SilentAimTarget then
            ...
            local characterAddedConnection = SilentAimModule.SilentAimTarget.CharacterAdded:Connect(function(newCharacter)
                print("Silent Aim: Target " .. SilentAimModule.SilentAimTarget.Name .. " has respawned/reset")
                ...
                if SilentAimModule.SilentAimTarget == SilentAimLockedTarget and SilentAimEnabled then
                    ...
                    if SilentAimModule.SilentAimTarget and SilentAimClosestPart then
                        ...
                        -- Re-enable ForceHitModule indicators after target respawn
                        if ForceHitModule then
                            ForceHitModule.TargetPlayer = SilentAimModule.SilentAimTarget
                            ForceHitModule.SelectedTarget = SilentAimModule.SilentAimTarget.Character:FindFirstChild(getgenv().Rake.Settings.AimPart or "Head")
                            ForceHitModule.Enabled = true
                            ForceHitModule.ManuallyEnabled = true
                            print("Silent Aim: ForceHitModule indicators re-enabled for target " .. SilentAimModule.SilentAimTarget.Name .. " after respawn")
                        end
                    end
                end
            end)
